...
def prologue(first_disp=0x08, second_disp=0x08, tail=b"\xc2\x08\x00"):
    ...
    return bytes([0x8B, 0x54, 0x24, first_disp, 0x56, 0x8B, 0x74, 0x24, second_disp]) + tail


ITEM = {"first_offset": 0, "second_offset": 5, "expected_rewritten_offsets": [3, 8]}
...
def test_refuses_a_third_load_of_an_exchanged_slot():
    ...
    body = (
        bytes([0x8B, 0x54, 0x24, 0x08, 0x56, 0x8B, 0x74, 0x24, 0x08, 0x8B, 0x44, 0x24, 0x08])
        + b"\xc2\x08\x00"
    )
...
def test_shape_validator_pins_the_declaration():
    ...
    bad = {
        "esp_argument_exchanges": [
            {"first_offset": 5, "second_offset": 0, "expected_rewritten_offsets": [3, 8]}
        ]
    }
