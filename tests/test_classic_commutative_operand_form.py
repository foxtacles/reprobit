...
def apply(body=BODY, sites=None, relocation_offsets=frozenset(), **kwargs):
    if sites is None:
        sites = [site()]
    return commutative_algorithms.apply_commutative_operand_form(
...
class CommutativeOperandFormTests(unittest.TestCase):
    def test_the_exchange_swaps_the_operands_and_nothing_else(self):
        ...
        self.assertEqual(proof["sites"][0]["expected_rewritten_offsets"], [2, 4])
        self.assertEqual(proof["sites"][0]["operation"], "fmul")

    def test_a_disp8_fadd_pair_exchanges_both_encodings(self):
        image, proof = apply(sites=[site(pair_offset=7, operation="fadd", offsets=(8, 9, 11, 12))])
        # fld [edx+4]; fadd [esi+8]  ->  fld [esi+8]; fadd [edx+4]
        self.assertEqual(image[7:13], bytes.fromhex("d94608d84204"))
    ...
    def test_a_width_mismatch_is_refused(self):
        ...
        with self.assertRaises(ByteIdentityError) as raised:
            apply(body=bytes(body))
        self.assertIn("m32 x87 memory binary operation", str(raised.exception))
    ...
    def test_compiler_state_may_reseat_only_the_pair_interior(self):
        source = bytes.fromhex("d901d84804c3")
        image, proof = apply(
            body=source,
            sites=[site(pair_offset=0, offsets=(1, 2, 3, 4))],
            allow_interior_boundary_reseat=True,
        )

        self.assertEqual(image, bytes.fromhex("d94004d809c3"))
        self.assertEqual(
            proof["kind"],
            "commutative_operand_form_interior_boundary_reseat_v1",
        )
        self.assertEqual(
            proof["interior_boundary_moves"],
            [
                {
                    "pair_offset": 0,
                    "seed_operator_offset": 2,
                    "image_operator_offset": 3,
                }
            ],
        )

    def test_compiler_state_may_reseat_the_interior_in_reverse(self):
        source = bytes.fromhex("d94004d809c3")
        image, proof = apply(
            body=source,
            sites=[site(pair_offset=0, offsets=(1, 2, 3, 4))],
            allow_interior_boundary_reseat=True,
        ...
        self.assertEqual(image, bytes.fromhex("d901d84804c3"))
        self.assertEqual(proof["sites"][0]["seed_operator_offset"], 3)
        self.assertEqual(proof["sites"][0]["image_operator_offset"], 2)

    def test_boundary_reseat_refuses_a_relocation_anywhere_in_the_pair(self):
        source = bytes.fromhex("d901d84804c3")
        for offset in range(5):
            with self.subTest(offset=offset), self.assertRaises(ByteIdentityError) as raised:
                apply(
                    body=source,
                    sites=[site(pair_offset=0, offsets=(1, 2, 3, 4))],
                    relocation_offsets=frozenset({offset}),
                    allow_interior_boundary_reseat=True,
                )
            self.assertIn("relocation lies inside the pair", str(raised.exception))

    def test_boundary_reseat_refuses_old_and_new_relocation_derived_entries(self):
        source = bytes.fromhex("d901d84804c3")
        for target in (2, 3):
            with self.subTest(target=target), self.assertRaises(ByteIdentityError) as raised:
                apply(
                    body=source,
                    sites=[site(pair_offset=0, offsets=(1, 2, 3, 4))],
                    internal_targets=frozenset({target}),
                    allow_interior_boundary_reseat=True,
                )
            self.assertIn("relocated target lies inside the pair", str(raised.exception))

    def test_boundary_reseat_refuses_an_external_entry_at_the_old_interior(self):
        source = bytes.fromhex("d901d84804c3")
        with self.assertRaises(ByteIdentityError) as raised:
            apply(
                body=source,
                sites=[site(pair_offset=0, offsets=(1, 2, 3, 4))],
                external_entries=frozenset({2}),
                allow_interior_boundary_reseat=True,
            )
        self.assertIn("external entry lies inside the pair", str(raised.exception))

    def test_boundary_reseat_refuses_an_external_entry_at_the_new_interior(self):
        source = bytes.fromhex("d901d84804c3")
        with self.assertRaises(ByteIdentityError) as raised:
            apply(
                body=source,
                sites=[site(pair_offset=0, offsets=(1, 2, 3, 4))],
                external_entries=frozenset({3}),
                allow_interior_boundary_reseat=True,
            )
        self.assertIn("not an instruction boundary", str(raised.exception))

    def test_boundary_reseat_refuses_computed_control_flow(self):
        source = bytes.fromhex("d901d84804ffe0")
        with self.assertRaises(ByteIdentityError) as raised:
            apply(
                body=source,
                sites=[site(pair_offset=0, offsets=(1, 2, 3, 4))],
                external_entries=frozenset({0}),
                allow_interior_boundary_reseat=True,
            )
        self.assertIn("computed control flow", str(raised.exception))
    ...
    def test_an_absolute_operand_is_refused(self):
        ...
        body = b"\x90" + bytes.fromhex("d90544332211") + bytes.fromhex("d808dec1d95decc3")
        with self.assertRaises(ByteIdentityError) as raised:
    ...
    def test_a_branch_into_the_operator_is_refused(self):
        ...
        # 11: c3         ret
        body = bytes.fromhex("7402d901d808dec1d95decc3")
        with self.assertRaises(ByteIdentityError) as raised:
    ...
    def declaration(self, forms):
        return {
            ...
            "expected_changed_offsets": sorted(
                {offset for form in forms for offset in form["expected_rewritten_offsets"]}
            ),
