...
def codeview_stream(body_length=SIZE, debug_start=DEBUG_START, debug_end=DEBUG_END):
...
def make_coff(
    *, body=BODY, relocations=RELOCATIONS, lines=LINE_ROWS, fpo_packed=0x14, debug_stream=None
):
    ...
    """
    debug_s = debug_stream if debug_stream is not None else codeview_stream(len(body))
    ...
    symbols = [
        (".text", 0, 1, 0, 3, _section_aux(len(body), len(reloc_rows), len(line_bytes) // 6, 2)),
...
def apply_fixture(body=BODY, regions=(REGION,), frame_pointer_free=True, offsets=RELOCATIONS):
    return register_algorithms.apply_register_bijection_reencoding(
        body,
        [dict(item) for item in regions],
        relocation_set(offsets),
        "fixture",
        relocation_map(offsets),
        None,
        None,
        frame_pointer_free,
    )
...
def function_record(seed_bytes, donor_bytes, image, proof, derived_detail, **overrides):
    ...
    record = {
        ...
        "retail_relocations": relocation_oracle(donor_bytes, proof["relocation_reseat"]),
        "register_bijection_reencoding": {
            "kind": register_algorithms.REGISTER_BIJECTION_REENCODING_KIND,
            "regions": [dict(REGION)],
            "expected_fpo_record": {
                "ulOffStart": 0,
                "cbProcSize": dp["raw_size"],
                "cdwLocals": 1,
                "cdwParams": 2,
                "cbProlog": 4,
                "cbRegs": 4,
            ...
            "expected_rewritten_field_offsets": proof["rewritten_field_offsets"],
            "expected_region_instruction_counts": proof["region_instruction_counts"],
            ...
            "expected_carried_code_symbols": derived_detail["carried_code_symbols"],
...
class ReEncodingPrimitiveTest(unittest.TestCase):
    ...
    def test_refuses_esp_in_sigma(self):
        region = {"start": 14, "end": 22, "mapping": {"esp": "edi", "edi": "esp"}}
        with self.assertRaises(ByteIdentityError) as caught:
    ...
    def test_refuses_overlapping_regions(self):
        regions = (
            {"start": 14, "end": 22, "mapping": dict(SIGMA)},
    ...
    def test_refuses_a_rewrite_that_overlaps_a_relocation(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection_reencoding(
                BODY,
                [dict(REGION)],
                relocation_set() | frozenset({15}),
                "fixture",
                relocation_map(),
                None,
                None,
...
class DerivedDonorTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.coff = coff_format.CoffObject(self.derived)
        self.primary = self.coff.function_section(TARGET_SYMBOL)

    def test_the_body_is_the_image(self):
        self.assertEqual(bytes(coff_format.coff_body(self.coff, self.primary)), self.image)
        self.assertEqual(self.primary["raw_size"], len(self.image))

    def test_the_line_rows_are_carried_through_the_boundary_map(self):
        table = coff_algorithms._coff_table_bytes(self.coff, self.primary, "lines")
        rows = [
            struct.unpack_from("<IH", table, index * 6)
    ...
    def test_the_relocation_offsets_are_the_proved_reseat(self):
        rows = coff_format.detailed_relocations(self.coff, self.primary)
        self.assertEqual([row["offset"] for row in rows], [6, 26])
        self.assertEqual([row["target"] for row in rows], [NIL_SYMBOL, NIL_SYMBOL])

    def test_the_procedure_record_and_fpo_follow_the_image(self):
        stream = bytes(
            coff_format.coff_body(
                self.coff, coff_algorithms._comdat_child(self.coff, self.primary, ".debug$S")
            )
        )
        self.assertEqual(
            struct.unpack_from("<III", stream, 16), (len(self.image), DEBUG_START, DEBUG_END + 2)
        )
        self.assertEqual(
            self.detail["procedure_range"], [len(self.image), DEBUG_START, DEBUG_END + 2]
        )
        fpo = bytes(
            coff_format.coff_body(
                self.coff, coff_algorithms._comdat_child(self.coff, self.primary, ".debug$F")
            )
        )
        record = debug_algorithms.parse_fpo_data(fpo, expected_proc_size=len(self.image))
        self.assertEqual(record["cbFrame"], 0)
        original = debug_algorithms.parse_fpo_data(
            bytes(
                coff_format.coff_body(
                    coff_format.CoffObject(self.donor),
                    coff_algorithms._comdat_child(
                        coff_format.CoffObject(self.donor),
                        coff_format.CoffObject(self.donor).function_section(TARGET_SYMBOL),
                        ".debug$F",
                    ),
                )
            ),
    ...
    def test_the_markers_and_auxiliaries_follow_the_image(self):
        _index, marker = coff_algorithms._coff_marker(self.coff, ".ef", self.primary["number"])
        self.assertEqual(marker["value"], len(self.image))
        function_index, symbol = coff_algorithms.function_symbol(
            self.coff, TARGET_SYMBOL, self.primary["number"]
        )
        auxiliary = coff_format.coff_auxiliary(self.coff, function_index, symbol)
        self.assertEqual(int.from_bytes(auxiliary[4:8], "little"), len(self.image))
        section_index, section_symbol = coff_algorithms._coff_section_symbol(
            self.coff, self.primary
        )
        section_aux = coff_format.coff_auxiliary(self.coff, section_index, section_symbol)
        self.assertEqual(int.from_bytes(section_aux[0:4], "little"), len(self.image))

    def test_the_object_topology_is_unchanged(self):
        donor = coff_format.CoffObject(self.donor)
        self.assertEqual(
            coff_algorithms.function_multiset(self.coff), coff_algorithms.function_multiset(donor)
        )
...
class ComposerTest(unittest.TestCase):
    ...
    def test_produces_the_declared_candidate_body(self):
        composed, detail = self._compose()
        coff = coff_format.CoffObject(composed)
        primary = coff.function_section(TARGET_SYMBOL)
        self.assertEqual(bytes(coff_format.coff_body(coff, primary)), self.image)
        self.assertEqual(
            detail["splice_class"], register_algorithms.REGISTER_BIJECTION_REENCODING_CLASS
        )
    ...
    def test_the_composed_object_keeps_every_other_comdat(self):
        ...
        self.assertEqual(
            coff_algorithms.function_multiset(coff), coff_algorithms.function_multiset(seed)
        )
        other = coff.function_section(OTHER_SYMBOL)
        self.assertEqual(bytes(coff_format.coff_body(coff, other)), b"OTHER-FN")
    ...
    def test_refuses_a_declaration_that_disagrees_with_the_measurement(self):
        ...
        record["register_bijection_reencoding"]["expected_growth"] = [[16, 16, 2, 3]]
        with self.assertRaises(ByteIdentityError) as caught:
    ...
    def test_refuses_a_stale_fpo_declaration(self):
        record = copy.deepcopy(self.record)
        record["register_bijection_reencoding"]["expected_fpo_record"]["cbRegs"] = 3
        with self.assertRaises(ByteIdentityError) as caught:
            self._compose(record)
        self.assertIn("FPO record differs", str(caught.exception))

    def test_refuses_when_the_object_declares_an_ebp_frame(self):
        seed, donor, image, proof, _derived, detail = composed_fixture(fpo_packed=0x14 | (1 << 6))
...
class ManifestValidationTest(unittest.TestCase):
    ...
    def _validate(self, spec=None, preimage=SIZE, image=None):
        return register_algorithms.validate_register_bijection_reencoding(
            spec or self.spec, "spec", preimage, len(self.image) if image is None else image
        )

    def test_accepts_the_measured_declaration(self):
        normalized = self._validate()
        self.assertEqual(normalized["regions"][0]["mapping"], {"ebp": "edi", "edi": "ebp"})
    ...
    def test_rejects_a_reseat_that_collides(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            self._validate(spec)
        self.assertIn("expected_relocation_reseat is invalid", str(caught.exception))
    ...
    def test_rejects_a_rewritten_field_outside_every_region(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            self._validate(spec)
        self.assertIn("expected_rewritten_field_offsets is invalid", str(caught.exception))
    ...
    def test_rejects_a_procedure_range_that_is_not_the_image(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            self._validate(spec)
        self.assertIn("expected_procedure_range is invalid", str(caught.exception))

    def test_rejects_unsorted_regions(self):
        spec = copy.deepcopy(self.spec)
        spec["regions"] = [
            {"start": 18, "end": 22, "mapping": dict(SIGMA)},
