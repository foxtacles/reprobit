...
def make_coff(
    *,
    ...
    for item in section_inputs:
        ...
        cursor += len(item["raw"])
        table = b"".join(struct.pack("<IIH", *row) for row in item["relocations"])
    ...
    checksum = int.from_bytes(hashlib.sha256(bytes(body)).digest()[:4], "little")
    symbols = [
        (
            ".text",
            0,
            ...
            0,
            3,
            _section_aux(len(body), len(reloc_rows), len(lines) // 6, 2, checksum=checksum),
        ),
        (".file", 0, -2, 0, 103, b"fixture.cpp\0".ljust(18, b"\0")),
        (TARGET_SYMBOL, 0, 1, 0x20, 2, _function_aux(len(body), sections[0]["line_offset"])),
        (".bf", 0, 1, 0, 101, _marker_aux(11)),
        (".ef", len(body), 1, 0, 101, _marker_aux(13)),
        (".debug$F", 0, 2, 0, 3, _section_aux(len(fpo), 0, 0, 5, associated=1)),
        (".debug$S", 0, 3, 0, 3, _section_aux(len(debug_s), 0, 0, 5, associated=1)),
        (nil_symbol, 0, 0, 0x00, 2, None),
        (".text", 0, 4, 0, 3, _section_aux(8, 0, 2, 2, checksum=0x1234)),
        (OTHER_SYMBOL, 0, 4, 0x20, 2, _function_aux(8, sections[3]["line_offset"])),
    ...
    for name, value, section, stype, storage, aux in symbols:
        table.extend(
            encoded(name)
            + struct.pack("<IhHBB", value, section, stype, storage, 1 if aux is not None else 0)
        )
        count += 1
        if aux is not None:
            table.extend(aux)
    ...
    for item in sections:
        ...
        headers.extend(
            struct.pack(
                ...
                item["line_offset"],
                len(item["relocations"]),
                len(item["lines"]) // 6,
                item["characteristics"],
            )
        )
    header = struct.pack("<HHIIIHH", 0x14C, len(sections), 0x1234, cursor, count, 0, 0)
    return bytes(header + headers + payload + table + strings)


def relocation_set(offsets=RELOCATIONS):
    return frozenset(offset + byte for offset in offsets for byte in range(4))


def sigma_image(body=BODY):
    image, _ = register_algorithms.apply_register_bijection(
...
def relocation_oracle(seed_bytes):
    ...
    rows = coff_format.detailed_relocations(coff, primary)
    return [
        {
            "offset": row["offset"],
            "type": row["type"],
...
def function_record(seed_bytes, donor_bytes, image, **overrides):
    ...
    donor_body = coff_format.coff_body(donor, dp)
    debug_child = coff_algorithms._comdat_child(seed, sp, ".debug$S")
    debug_stream = coff_format.coff_body(seed, debug_child)
    declared = [
        {
            "name": "_Y",
            "record_offset": next(
                record["offset"]
                for record in debug_algorithms.parse_codeview_symbol_stream(debug_stream, "x")
    ...
    debug_image = register_algorithms.apply_codeview_register_bijection(
        debug_stream, SIGMA, declared, "x"
    )
    record = {
        "mangled": TARGET_SYMBOL,
        "donor": "d_0123456789ab",
        "splice_class": register_algorithms.REGISTER_BIJECTION_CLASS,
        ...
        "retail_oracle": {
            "image": "SAMPLE.DLL",
            "address": f"0x{RETAIL_ADDRESS:08x}",
            "verdict": "MATCH",
            "length": len(image),
        },
        "retail_relocations": relocation_oracle(seed_bytes),
        "register_bijection": {
            "kind": register_algorithms.REGISTER_BIJECTION_KIND,
            "mapping": dict(SIGMA),
            ...
            "expected_seed_debug_s_sha256": foundation_algorithms.sha256_bytes(debug_stream),
            "expected_image_debug_s_sha256": foundation_algorithms.sha256_bytes(debug_image),
            "authenticity_rationale": "A bijective renaming of general registers proved dead on "
...
class RegisterBijectionImageTests(unittest.TestCase):
    ...
    def test_refuses_a_bijection_that_cannot_preserve_a_length(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                BODY, {"esi": "esp", "esp": "esi"}, REGION, relocation_set(), "fixture"
            )
        self.assertIn("ESP", str(caught.exception))
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                BODY, {"ebx": "ebp", "ebp": "ebx"}, REGION, relocation_set(), "fixture"
            )
    ...
    def test_refuses_an_instruction_with_an_implicit_operand(self):
        ...
        for encoding, name in ((b"\x99", "cdq"), (b"\xf7\xf9", "idiv")):
            body = bytes.fromhex("538b0d00000000") + encoding + bytes.fromhex("e8000000005bc3")
            ...
            with self.assertRaises(ByteIdentityError) as caught:
                register_algorithms.apply_register_bijection(
                    ...
                    CALL_SIGMA,
                    (1, 7 + len(encoding)),
                    frozenset(offset + byte for offset in (3, call_at) for byte in range(4)),
                    name,
                    {
    ...
    def test_an_unadmitted_group_member_is_still_outside_the_table(self):
        ...
        for encoding, name in (
            (b"\xf6\xe1", "mul r/m8"),
            (b"\xff\xe8", "far jmp"),
            (b"\x66\x0f\xaf\xc1", "imul r16, r/m16"),
            (b"\xaa\x90", "stosb"),
        ):
            body = bytes.fromhex("538b0d00000000") + encoding + bytes.fromhex("e8000000005bc3")
            with self.assertRaises(ByteIdentityError) as caught:
                register_algorithms.apply_register_bijection(
                    ...
                    CALL_SIGMA,
                    CALL_REGION,
                    CALL_RELOCATION_OFFSETS,
                    name,
                    call_relocations(CDECL_SYMBOL),
                )
            self.assertIn("outside the register-bijection table", str(caught.exception), name)

    def test_refuses_a_rewrite_that_overlaps_a_relocation(self):
        """A relocated operand belongs to the linker, not to sigma."""
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                BODY, SIGMA, REGION, relocation_set() | {4}, "fixture"
            )
    ...
    def test_refuses_a_region_whose_registers_are_live_on_exit(self):
        ...
        self.assertIn("live on an edge leaving the region", str(caught.exception))
...
class CodeViewRegisterMapTests(unittest.TestCase):
    ...
    def test_maps_only_the_declared_register_records(self):
        ...
        declared = [
            {
                ...
                "record_offset": next(
                    record["offset"]
                    for record in debug_algorithms.parse_codeview_symbol_stream(stream, "x")
                    if record["type"] == 0x0002 and record["name"] == "_Y"
                ),
                "donor_register": "edi",
                "image_register": "ebx",
        ...
        image = register_algorithms.apply_codeview_register_bijection(stream, SIGMA, declared, "x")
        self.assertEqual(len(image), len(stream))
        records = debug_algorithms.parse_codeview_symbol_stream(image, "x")
        registers = [
            int.from_bytes(image[record["offset"] + 6 : record["offset"] + 8], "little")
    ...
    def test_refuses_an_undeclared_register_record(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_codeview_register_bijection(stream, SIGMA, [], "x")
...
class RegisterBijectionCompositionTests(unittest.TestCase):
    ...
    def test_positive_control_produces_the_declared_candidate(self):
        ...
        primary = coff.function_section(TARGET_SYMBOL)
        self.assertEqual(coff_format.coff_body(coff, primary), self.image)
        ...
        self.assertEqual(coff_format.coff_body(coff, primary)[:3], BODY[:3])
        # and the CodeView record now names EBX
        child = coff_algorithms._comdat_child(coff, primary, ".debug$S")
        stream = coff_format.coff_body(coff, child)
        registers = [
            int.from_bytes(stream[record["offset"] + 6 : record["offset"] + 8], "little")
            for record in debug_algorithms.parse_codeview_symbol_stream(stream, "x")
            if record["type"] == 0x0002
        ]
    ...
    def test_refuses_a_donor_whose_body_is_not_the_pinned_pre_image(self):
        ...
        coff = coff_format.CoffObject(self.donor)
        primary = coff.function_section(TARGET_SYMBOL)
    ...
    def test_schema_refuses_an_image_pin_equal_to_the_donor(self):
        ...
        normalized = register_algorithms.validate_register_bijection(spec, "spec", SIZE)
        self.assertEqual(normalized["mapping"], {"ebx": "edi", "edi": "ebx"})
        for broken, message in (
            ({**spec, "mapping": {"ebx": "ebp", "ebp": "ebx"}}, "ESP or EBP"),
            ({**spec, "mapping": {"ebx": "ebx", "edi": "edi"}}, "fixed-point-free"),
            ({**spec, "region_start": 0}, "region_start"),
        ):
            with self.assertRaises(ByteIdentityError) as caught:
                register_algorithms.validate_register_bijection(broken, "spec", SIZE)
...
CDECL_SYMBOL = "?CreateCursorSurface@FixtureSurface@@SAPAUIFixture@@XZ"
THISCALL_SYMBOL = "?PlaceItem@SampleWorld@@QAEJPAVSampleItem@@@Z"
...
# 14: 5b              pop ebx
# 15: c3              ret
CALL_BODY = bytes.fromhex("538b0d000000008b01e8000000005bc3")
CALL_REGION = (1, 9)
CALL_SIGMA = {"eax": "ecx", "ecx": "eax"}
CALL_RELOCATION_OFFSETS = frozenset(offset + byte for offset in (3, 10) for byte in range(4))


def call_relocations(callee, *, extra=()):
    rows = {3: {"width": 4, "target": NIL_SYMBOL}, 10: {"width": 4, "target": callee}}
    rows.update(extra)
    return rows
...
class CallArgumentModelTests(unittest.TestCase):
    ...
    def test_decoration_table_reads_the_conventions_it_knows(self):
        for symbol, expected in (
            ...
            ("?GlobalManager@@YAPAVFixtureManager@@XZ", frozenset()),
            (STDCALL_SYMBOL, frozenset()),
            (THISCALL_SYMBOL, frozenset({"ecx"})),
            ("?Insert@SampleList@@QAEXHW4Mode@Element@@@Z", frozenset({"ecx"})),
    ...
    def test_a_thiscall_callee_keeps_ecx_live_and_refuses(self):
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                ...
                CALL_RELOCATION_OFFSETS,
                "thiscall",
                call_relocations(THISCALL_SYMBOL),
            )
        self.assertIn("['ecx'] is live on an edge leaving the region", str(caught.exception))
    ...
    def test_an_unrecognised_decoration_stays_conservative(self):
        for callee in (UNMANGLED_SYMBOL, THUNK_SYMBOL):
            ...
            self.assertIn("is live on an edge leaving the region", str(caught.exception))
    ...
    def test_an_indirect_call_can_never_be_named_and_stays_conservative(self):
        ...
        body = bytes.fromhex("538b0d000000008b01ff15000000005bc3")
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                body,
                CALL_SIGMA,
                ...
                CALL_RELOCATION_OFFSETS,
                "indirect",
                {3: {"width": 4, "target": NIL_SYMBOL}, 11: {"width": 4, "target": CDECL_SYMBOL}},
            )
        self.assertIn("is live on an edge leaving the region", str(caught.exception))
    ...
    def test_an_unrelocated_call_stays_conservative(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                CALL_BODY, CALL_SIGMA, CALL_REGION, CALL_RELOCATION_OFFSETS, "no-relocations"
            )
        self.assertIn("is live on an edge leaving the region", str(caught.exception))

    def test_a_call_result_that_is_read_stays_live(self):
        """The clobber set may never hide a return value from liveness."""
        body = bytes.fromhex("53e8000000008bc88b195bc3")
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                body,
                CALL_SIGMA,
    ...
    def test_an_indirect_jump_needs_the_relocated_in_body_target_set(self):
        ...
        """
        body = bytes.fromhex("538b0d000000008b01ffe05bc3")
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                body,
                CALL_SIGMA,
    ...
    def test_a_relocated_in_body_target_inside_the_region_is_refused(self):
        ...
        body = bytes.fromhex("53ffe08b0d000000008b01e8000000005bc3")
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                body,
                CALL_SIGMA,
                (3, 11),
                frozenset(offset + byte for offset in (5, 12) for byte in range(4)),
                "table-target",
                {5: {"width": 4, "target": NIL_SYMBOL}, 12: {"width": 4, "target": CDECL_SYMBOL}},
                None,
                frozenset({9}),
            )
        self.assertIn("enters the region other than at its first", str(caught.exception))
...
class WidenedSemanticTableTests(unittest.TestCase):
    ...
    def test_an_x87_memory_form_rewrites_only_its_address_register(self):
        ...
        body = bytes.fromhex("538b0d00000000d901d919e8000000005bc3")
        image, proof = register_algorithms.apply_register_bijection(
            body,
            {"ecx": "edx", "edx": "ecx"},
            (1, 11),
            frozenset(offset + byte for offset in (3, 12) for byte in range(4)),
            "x87",
            {3: {"width": 4, "target": NIL_SYMBOL}, 12: {"width": 4, "target": CDECL_SYMBOL}},
        )
    ...
    def test_refuses_a_sub_register_field_sigma_cannot_rewrite(self):
        ...
        """
        body = bytes.fromhex("538b0d000000008a01e8000000005bc3")
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                body,
                CALL_SIGMA,
        ...
        self.assertIn("is named by a sub-register field", str(caught.exception))
    ...
    def test_refuses_an_implicit_operand_form_inside_a_widened_group(self):
        ...
        """
        body = bytes.fromhex("538b0d00000000") + b"\xf6\xe1" + bytes.fromhex("e8000000005bc3")
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                body,
                CALL_SIGMA,
        ...
        body = bytes.fromhex("538b0d00000000") + b"\xf7\xf1" + bytes.fromhex("e8000000005bc3")
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                body,
                CALL_SIGMA,
    ...
    def test_refuses_an_operand_size_prefix_on_an_unadmitted_form(self):
        ...
        body = bytes.fromhex("538b0d0000000066f7d9e8000000005bc3")
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                body,
                CALL_SIGMA,
        ...
        self.assertIn("operand-size prefix is outside", str(caught.exception))
...
class ExternalTailJumpTests(unittest.TestCase):
    ...
    FUNCLET_BODY = bytes.fromhex("538b0d000000008b01e8000000005bc3e900000000")
...
class DelegateSelectionTests(unittest.TestCase):
    ...
    def test_the_pins_alone_name_the_delegate(self):
        fpo = register_algorithms.REGISTER_BIJECTION_FPO_CLOSURE
        eh = register_algorithms.REGISTER_BIJECTION_EH_CLOSURE
        choose = register_algorithms.register_bijection_delegate
        ...
        self.assertEqual(choose(fpo, [[163, "T"]]), "equal_body_eh_structural_local")
        self.assertEqual(choose(eh, []), "equal_body_eh_structural_local")
        self.assertEqual(choose(eh, [[12, "L"]]), "equal_body_eh_structural_local")
    ...
    def test_refuses_a_closure_pin_that_disagrees_with_the_objects(self):
        ...
        function = function_record(
            ...
            donor,
            image,
            expected_closure=register_algorithms.REGISTER_BIJECTION_EH_CLOSURE,
...
class PinnedRenameTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.symbols = [[offset, LOCAL_SYMBOL_SEED, LOCAL_SYMBOL_DONOR] for offset in RELOCATIONS]

    def _record(self, **overrides):
        return function_record(self.seed, self.donor, self.image, **overrides)
    ...
    def test_a_declared_object_local_rename_composes(self):
        ...
        self.assertEqual(
            {row["target"] for row in coff_format.detailed_relocations(coff, primary)},
            {LOCAL_SYMBOL_SEED},
        )
    ...
    def test_refuses_a_rename_whose_symbol_pair_is_wrong(self):
        ...
        function = self._record(
            ...
            expected_code_rename_symbols=[
                [offset, LOCAL_SYMBOL_SEED, "$T99999"] for offset in RELOCATIONS
            ],
            expected_xdata_rename_offsets=[],
        )
        with self.assertRaises(ByteIdentityError) as caught:
...
#
# `Sample::Enable` forced three additions at once: a COMDAT whose body ends in
...
TAIL_BODY = bytes.fromhex("538b0d000000008b0133c0c3") + b"\x00" * 4
TAIL_RELOCATIONS = {3: {"width": 4, "target": NIL_SYMBOL}, 12: {"width": 4, "target": "$L12345"}}
TAIL_OFFSETS = frozenset(offset + byte for offset in (3, 12) for byte in range(4))
...
class DataTailTests(unittest.TestCase):
    ...
    def test_refuses_a_region_that_reaches_past_the_code(self):
        ...
        self.assertIn("region reaches past the body's code", str(caught.exception))

    def test_the_data_tail_is_never_rewritten(self):
        image, proof = register_algorithms.apply_register_bijection(
            TAIL_BODY, CALL_SIGMA, (1, 9), TAIL_OFFSETS, "tail", TAIL_RELOCATIONS, 12, frozenset()
        )
        self.assertEqual(proof["code_length"], 12)
        self.assertEqual(image[12:], TAIL_BODY[12:])
        self.assertTrue(all(offset < 12 for offset in proof["rewritten_offsets"]))
...
class SubRegisterLivenessTests(unittest.TestCase):
    ...
    def test_fnstsw_ax_defines_the_bits_test_ah_reads(self):
        """The refinement that `Sample::Enable` needs, in eight instructions.
        ...
        """
        body = bytes.fromhex("538b0d000000008b01dfe0f6c40133c05bc3")
        ...
        self.assertEqual(status["write_atoms"], frozenset({"eax.l", "eax.h"}))
    ...
    def test_a_byte_definition_does_not_kill_the_other_byte(self):
        ...
        body = bytes.fromhex("538b0d000000008b018a03f6c40133c05bc3")
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_register_bijection(
                body,
                CALL_SIGMA,
    ...
    def test_a_byte_definition_kills_its_own_byte(self):
        ...
        body = bytes.fromhex("538b0d000000008b018a03f6c00133c05bc3")
        _image, proof = register_algorithms.apply_register_bijection(
            body,
            CALL_SIGMA,
            (1, 9),
...
class RelocLayoutDelegateTests(unittest.TestCase):
    ...
    def test_a_non_empty_move_pin_names_the_reloc_layout_delegate(self):
        self.assertEqual(
            register_algorithms.register_bijection_delegate(
                register_algorithms.REGISTER_BIJECTION_EH_CLOSURE, [[1, "T"]], [[10, 12]]
            ),
    ...
    def test_an_empty_move_pin_can_never_reach_it(self):
        for closure, renames in (
            (register_algorithms.REGISTER_BIJECTION_FPO_CLOSURE, []),
            (register_algorithms.REGISTER_BIJECTION_EH_CLOSURE, [[1, "T"]]),
        ):
            self.assertNotEqual(
                register_algorithms.register_bijection_delegate(closure, renames, []),
                "equal_body_eh_reloc_layout",
            )
            self.assertNotEqual(
                register_algorithms.register_bijection_delegate(closure, renames),
...
class CodeViewDonorStreamTests(unittest.TestCase):
    ...
    def test_every_record_takes_the_donor_register(self):
        ...
        image = register_algorithms.apply_codeview_register_bijection(
    ...
    def test_refuses_a_donor_stream_that_differs_structurally(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_codeview_register_bijection(
                seed, SIGMA, [], "map", bytes(donor)
            )
        self.assertIn("differs outside its S_REGISTER fields", str(caught.exception))

    def test_refuses_a_donor_stream_whose_record_list_differs(self):
        seed = codeview_stream((23, 24))
        donor = codeview_stream((23, 24, 20))
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_codeview_register_bijection(seed, SIGMA, [], "map", donor)
    ...
    def test_refuses_a_declaration_whose_donor_register_is_wrong(self):
        seed = codeview_stream((23, 24))
        records = debug_algorithms.parse_codeview_symbol_stream(seed, "map")
        register = next(
            item
            for item in records
            if item["type"] == register_algorithms.CODEVIEW_REGISTER_RECORD_TYPE
        )
        with self.assertRaises(ByteIdentityError) as caught:
            register_algorithms.apply_codeview_register_bijection(
                seed,
    ...
    def test_the_closed_record_table_parses_s_ldata32(self):
        """`Sample::Enable`'s stream carries two empty-named S_LDATA32 records."""
        record = bytes([11, 0, 0x01, 0x02]) + bytes(9)
        parsed = debug_algorithms.parse_codeview_symbol_stream(record, "ldata")
