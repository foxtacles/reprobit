...
The fixture is a representative classic compiler relocation window:
...
class ReseatDerivationTests(unittest.TestCase):
    ...
    def test_the_declared_reseat_is_the_permutation_s_own(self):
        ...
        self.assertEqual(proof["relocation_reseat"], [[RELOCATION_OFFSET, RESEATED_OFFSET]])
        self.assertEqual(
            proof["windows"][0]["relocation_reseat"], [[RELOCATION_OFFSET, RESEATED_OFFSET]]
        )
        # the operand bytes travel with their instruction and nothing else
        self.assertEqual(
            image[RESEATED_OFFSET : RESEATED_OFFSET + 4],
    ...
    def test_a_reseat_declared_with_no_relocation_inside_refuses(self):
        ...
        self.assertIn("no relocation operand lies inside the window", str(caught.exception))
    ...
    def test_a_relocation_straddling_an_instruction_boundary_refuses(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            apply(windows=[window([[START + 5, START + 8]])], covered=covered, symbols=symbols)
        self.assertIn("straddles an instruction boundary", str(caught.exception))
...
class ReseatSchemaTests(unittest.TestCase):
    ...
    def test_a_valid_reseat_is_normalised(self):
        ...
        self.assertEqual(
            value["windows"][0]["relocation_reseat"], [[RELOCATION_OFFSET, RESEATED_OFFSET]]
        )
    ...
    def test_an_offset_outside_the_window_is_refused(self):
        for bad in (
            ...
            [[RELOCATION_OFFSET, RESEATED_OFFSET], [RELOCATION_OFFSET, START]],
            [],
        ):
...
class ReseatDelegateTests(unittest.TestCase):
    ...
    def test_a_reseat_selects_the_reloc_layout_delegate(self):
        for closure in (
            schedule_algorithms.INSTRUCTION_SCHEDULE_FPO_CLOSURE,
            schedule_algorithms.INSTRUCTION_SCHEDULE_EH_CLOSURE,
        ):
            for renames in ([], [[12, "L"]]):
                with self.subTest(closure=closure, renames=renames):
                    self.assertEqual(
                        schedule_algorithms.instruction_schedule_delegate(closure, renames, True),
                        "equal_body_eh_reloc_layout",
                    )

    def test_without_a_reseat_the_delegates_are_unchanged(self):
        self.assertEqual(
            schedule_algorithms.instruction_schedule_delegate(
                schedule_algorithms.INSTRUCTION_SCHEDULE_FPO_CLOSURE, []
            ),
            "equal_body_strict",
        )
