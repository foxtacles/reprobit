...
def relocation_map(body=BODY):
    return {offset: {"width": 4, "target": fixture.NIL_SYMBOL} for offset in RELOCATIONS}


def reversed_image(body=BODY, sites=None, relocations=None, external=frozenset()):
    image, proof = relational_algorithms.apply_relational_form(
        body,
        sites if sites is not None else SITES,
        fixture.relocation_set(),
        "fixture",
        relocation_map(body),
        None,
        external,
...
class MirrorTableTests(unittest.TestCase):
    ...
    def test_zf_is_the_only_flag_the_reversal_preserves(self):
        ...
        for flag in ("cf", "pf", "af", "zf", "sf", "of"):
            if all(
                _sub_flags(a, b, self.WIDTH)[flag] == _sub_flags(b, a, self.WIDTH)[flag]
                for a, b in self.DOMAIN
            ):
                preserved.add(flag)
        self.assertEqual(preserved, set(relational_algorithms.IA32_RELATIONAL_PRESERVED_FLAGS))
        self.assertEqual(
            set(relational_algorithms.IA32_ARITHMETIC_FLAGS) - preserved,
            set(relational_algorithms.IA32_RELATIONAL_CHANGED_FLAGS),
        )
    ...
    def test_the_mirror_table_is_the_exhaustive_one(self):
        ...
        for left in _CONDITION:
            agreeing = [
                right
                for right in _CONDITION
                if all(
                    bool(_CONDITION[left](_sub_flags(a, b, self.WIDTH)))
                    == bool(_CONDITION[right](_sub_flags(b, a, self.WIDTH)))
                    for a, b in self.DOMAIN
                )
            ]
        ...
        self.assertEqual(derived, relational_algorithms.IA32_RELATIONAL_MIRROR)

    def test_the_six_unmirrorable_conditions_are_absent(self):
        for name in ("o", "no", "s", "ns", "p", "np"):
            self.assertNotIn(name, relational_algorithms.IA32_RELATIONAL_MIRROR)

    def test_the_mirror_is_an_involution(self):
        for left, right in relational_algorithms.IA32_RELATIONAL_MIRROR.items():
            self.assertEqual(relational_algorithms.IA32_RELATIONAL_MIRROR[right], left)

    def test_each_condition_reads_the_flags_the_table_says(self):
        for code, flags in relational_algorithms.IA32_CONDITION_FLAGS.items():
            name = relational_algorithms.IA32_CONDITION_NAMES[code]
            for flag in ("cf", "pf", "af", "zf", "sf", "of"):
                if flag in flags:
                    continue
...
class RelationalImageTests(unittest.TestCase):
    ...
    def test_positive_control_reverses_exactly_two_bytes(self):
        ...
        self.assertEqual(proof["sites"][0]["seed_compare_opcode"], 0x3B)
        self.assertEqual(proof["sites"][0]["image_compare_opcode"], 0x39)

    def test_register_direct_reencoding_reports_the_opcode_it_keeps(self):
        body = bytes.fromhex("3bc17501c3c3")  # cmp eax, ecx / jne +1 / ret / ret
        image, proof = relational_algorithms.apply_relational_form(
            body,
            [
                {
            ...
            frozenset(),
            "register-direct fixture",
        )

        self.assertEqual(image, bytes.fromhex("3bc87501c3c3"))
        self.assertEqual(proof["sites"][0]["seed_compare_opcode"], 0x3B)
        self.assertEqual(proof["sites"][0]["image_compare_opcode"], 0x3B)
    ...
    def test_admits_a_successor_that_reads_only_zf(self):
        ...
        image, proof = relational_algorithms.apply_relational_form(
            ...
            frozenset(),
            "fixture",
            {offset: {"width": 4, "target": fixture.NIL_SYMBOL} for offset in (5, 13)},
        )
    ...
    def test_refuses_a_compare_against_an_immediate(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            relational_algorithms.apply_relational_form(
                bytes(body),
                SITES,
                frozenset(),
                "fixture",
                {offset: {"width": 4, "target": fixture.NIL_SYMBOL} for offset in (5, 21)},
            )
...
class FlagModelTests(unittest.TestCase):
    def test_an_unmodelled_opcode_reads_every_flag(self):
        ...
        items, _, _ = relational_algorithms.ia32_relational_flow_walk(body, None, "fixture")
        self.assertEqual(items[0]["reads_flags"], relational_algorithms.IA32_ARITHMETIC_FLAGS)
    ...
    def test_the_group_digit_separates_cmp_from_adc(self):
        ...
        compare, _, _ = relational_algorithms.ia32_relational_flow_walk(
            bytes.fromhex("83ff00c3"), None, "fixture"
        )
        adc, _, _ = relational_algorithms.ia32_relational_flow_walk(
        ...
        self.assertEqual(compare[0]["writes_flags"], relational_algorithms.IA32_ARITHMETIC_FLAGS)
    ...
    def test_a_variable_shift_is_not_credited_with_writing_a_flag(self):
        ...
        items, _, _ = relational_algorithms.ia32_relational_flow_walk(
    ...
    def test_a_repeat_prefix_must_prefix_a_string_opcode(self):
        ...
        with self.assertRaises(ByteIdentityError):
            relational_algorithms.ia32_relational_flow_walk(
    ...
    def test_refuses_an_unmodellable_opcode(self):
        with self.assertRaises(ByteIdentityError):
            relational_algorithms.ia32_relational_flow_walk(
                bytes.fromhex("f040c3"), None, "fixture"
            )
...
class ExternalEntryTests(unittest.TestCase):
    ...
    ISLAND = bytes.fromhex("c333c0c3")
    FLAGGED = bytes.fromhex("c31cffc3")  # sbb al, -1 reads CF

    def test_unreachable_code_refuses_without_the_entry_set(self):
        with self.assertRaises(ByteIdentityError) as caught:
            relational_algorithms.ia32_relational_flow_walk(self.ISLAND, None, "fixture")
        self.assertIn("reachable neither from the entry", str(caught.exception))

    def test_the_declared_entry_closes_the_graph(self):
        items, _, entries = relational_algorithms.ia32_relational_flow_walk(
...
def declaration(**overrides):
    value = {
        "kind": relational_algorithms.RELATIONAL_FORM_KIND,
        ...
        "authenticity_rationale": "An operand exchange whose changed flags are proved dead at "
...
class RelationalSchemaTests(unittest.TestCase):
    def test_positive_control(self):
        normalized = relational_algorithms.validate_relational_form(declaration(), "x", len(BODY))
        self.assertEqual(normalized["sites"][0]["image_condition"], "g")

    def test_refuses_a_non_mirror_pair(self):
        with self.assertRaises(ByteIdentityError):
            relational_algorithms.validate_relational_form(
                declaration(sites=[dict(SITES[0], image_condition="ge")]), "x", len(BODY)
            )

    def test_refuses_an_unmirrorable_condition(self):
        with self.assertRaises(ByteIdentityError):
            relational_algorithms.validate_relational_form(
                declaration(sites=[dict(SITES[0], seed_condition="s", image_condition="s")]),
                "x",
                len(BODY),
            )
    ...
    def test_refuses_a_rewritten_offset_count_that_is_not_two_per_site(self):
        with self.assertRaises(ByteIdentityError):
            relational_algorithms.validate_relational_form(
                declaration(expected_rewritten_offsets=[COMPARE_AT]), "x", len(BODY)
            )
    ...
    def test_refuses_a_branch_before_its_compare(self):
        with self.assertRaises(ByteIdentityError):
            relational_algorithms.validate_relational_form(
                declaration(
                    sites=[dict(SITES[0], compare_offset=BRANCH_AT, branch_offset=COMPARE_AT)]
                ),
                "x",
                len(BODY),
            )
...
def function_record(seed_bytes, donor_bytes, image, **overrides):
    ...
    record = {
        ...
        "splice_class": relational_algorithms.RELATIONAL_FORM_CLASS,
        ...
        "retail_oracle": {
            "image": "SAMPLE.DLL",
            "address": f"0x{fixture.RETAIL_ADDRESS:08x}",
            "verdict": "MATCH",
            "length": len(image),
        },
...
class RelationalCompositionTests(unittest.TestCase):
    ...
    def test_positive_control_produces_the_declared_candidate(self):
        composed, detail = relational_algorithms.produce_relational_form_candidate(
            self.seed, self.seed, function_record(self.seed, self.seed, self.image)
        )
        ...
        self.assertEqual(detail["changed_flags"], ["af", "cf", "of", "pf", "sf"])

    def test_candidate_api_refuses_a_literal_oracle_argument(self):
        wrong = bytearray(self.retail)
        wrong[9] ^= 0xFF
        with self.assertRaises(TypeError):
            relational_algorithms.produce_relational_form_candidate(
                self.seed,
                self.seed,
    ...
    def test_refuses_a_donor_whose_body_is_not_the_pinned_pre_image(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            relational_algorithms.produce_relational_form_candidate(
                self.seed, donor, function_record(self.seed, self.seed, self.image)
            )
    ...
    def test_refuses_a_declaration_that_understates_the_rewrite(self):
        ...
        record["relational_form"] = declaration(
            ...
            expected_seed_debug_s_sha256=record["relational_form"]["expected_seed_debug_s_sha256"],
        )
        with self.assertRaises(ByteIdentityError) as caught:
            relational_algorithms.produce_relational_form_candidate(self.seed, self.seed, record)
    ...
    def test_the_output_keeps_the_seed_debug_and_line_tables(self):
        composed, _ = relational_algorithms.produce_relational_form_candidate(
            self.seed, self.seed, function_record(self.seed, self.seed, self.image)
        )
        ...
        for child in (".debug$F", ".debug$S"):
            self.assertEqual(
                coff_format.coff_body(out, coff_algorithms._comdat_child(out, op, child)),
                coff_format.coff_body(seed, coff_algorithms._comdat_child(seed, sp, child)),
            )
        self.assertEqual(
            coff_algorithms._coff_table_bytes(out, op, "lines"),
    ...
    def test_schema_refuses_an_image_pin_equal_to_the_donor(self):
        ...
        with self.assertRaises(ByteIdentityError):
            require(
                record["expected_body_sha256"] != record["expected_donor_body_sha256"],
                f"{manifest_context}: the image pin does not move the donor body",
            )
...
class CertificateCompositionTests(unittest.TestCase):
    ...
    SIGMA: ClassVar[dict[str, str]] = {"ebx": "edi", "edi": "ebx"}
    REGION = (3, 27)

    def sigma_image(self, body=BODY):
        image, _ = register_algorithms.apply_register_bijection(
            body, self.SIGMA, self.REGION, fixture.relocation_set(), "sigma"
        )
        return image

    def test_neither_operation_alone_reaches_the_target(self):
        target, _ = relational_algorithms.apply_relational_form(
            self.sigma_image(), SITES, fixture.relocation_set(), "both", relocation_map()
        )
        ...
        self.assertEqual(target[COMPARE_AT : COMPARE_AT + 2], bytes.fromhex("391d"))  # cmp [m], ebx
        self.assertEqual(target[BRANCH_AT], 0x7F)  # jg

    def test_the_two_operations_commute_exactly(self):
        sigma_then_reversal, _ = relational_algorithms.apply_relational_form(
            self.sigma_image(),
            SITES,
            fixture.relocation_set(),
            "sigma-then-reversal",
    ...
    def test_each_proof_survives_the_other_operation(self):
        ...
        _after, after_proof = relational_algorithms.apply_relational_form(
            self.sigma_image(), SITES, fixture.relocation_set(), "after", relocation_map()
        )
        self.assertEqual(
            plain_proof["sites"][0]["flags_live_out"], after_proof["sites"][0]["flags_live_out"]
        )
        self.assertEqual(plain_proof["rewritten_offsets"], after_proof["rewritten_offsets"])
        # and sigma's own byte set is the same before and after the reversal
        before = {index for index in range(len(BODY)) if BODY[index] != self.sigma_image()[index]}
        later = {
            index for index in range(len(BODY)) if plain[index] != self.sigma_image(plain)[index]
        }
        self.assertEqual(before, later)
        self.assertFalse(before & set(plain_proof["rewritten_offsets"]))

    def test_a_candidate_producer_does_not_read_the_later_target(self):
        ...
        coff = coff_format.CoffObject(composed)
        primary = coff.function_section(TARGET_SYMBOL)
        self.assertEqual(coff_format.coff_body(coff, primary), self.sigma_image())
        self.assertTrue(detail["candidate_only"])
