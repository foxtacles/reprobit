...
class RelationalImageTests(unittest.TestCase):
    ...
    def test_decodes_the_body_to_exhaustion(self):
        items, _successors, entries = byte_identity.ia32_relational_flow_walk(
...
class RelationalCompositionTests(unittest.TestCase):
    ...
    def test_positive_control_produces_the_declared_candidate(self):
        composed, detail = (
            byte_identity.produce_relational_form_candidate(
                self.seed, self.seed,
    ...
    def test_the_output_keeps_the_seed_debug_and_line_tables(self):
        composed, _ = byte_identity.produce_relational_form_candidate(
            self.seed, self.seed,
            function_record(self.seed, self.seed, self.image))
...
class CertificateCompositionTests(unittest.TestCase):
    ...
    def test_each_proof_survives_the_other_operation(self):
        ...
        _after, after_proof = byte_identity.apply_relational_form(
    ...
    def test_a_candidate_producer_does_not_read_the_later_target(self):
        ...
        composed, detail = byte_identity.produce_register_bijection_candidate(
            seed, seed, record)
        coff = byte_identity.CoffObject(composed)
        primary = coff.function_section(TARGET_SYMBOL)
        self.assertEqual(byte_identity.coff_body(coff, primary),
                         self.sigma_image())
        self.assertTrue(detail["candidate_only"])
