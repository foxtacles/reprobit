...
def declaration(**overrides):
    item = {
        ...
        "field_rewrites": [[0, 0, "edx"], [1, 0, "ecx"], [2, 0, "edx"], [3, 0, "ecx"]],
...
class RewriteTests(unittest.TestCase):
    def test_a_proved_rewrite_reaches_the_permuted_renamed_region(self):
        image, proof = apply()
        expected = bytes.fromhex("908b4df48b55f08955ec894de833c933d2c3")
        self.assertEqual(image, expected)
        self.assertEqual(proof["regions"][0]["dead_registers"], ["ecx", "edx"])
    ...
    def test_a_live_renamed_register_is_refused(self):
        ...
        self.assertIn("live on the region's exit edge", str(raised.exception))
    ...
    def test_an_instruction_outside_the_simulator_set_is_refused(self):
        ...
        with self.assertRaises(ByteIdentityError) as raised:
            apply(items=[declaration(region_end=18, target_order=[1, 0, 2, 3, 4, 5, 6])])
        self.assertIn("a return at 17 outside any inlined callee", str(raised.exception))
    ...
    def test_a_moved_relocation_is_reseated_with_its_instruction(self):
        body = bytes.fromhex("90c745f0112233448b55f433d2c3")
