...
def record(kind, payload):
    return (len(payload) + 2).to_bytes(2, "little") + kind.to_bytes(2, "little") + payload
...
def reg_local(typind, reg, name):
    return record(
        0x0002, typind.to_bytes(2, "little") + reg.to_bytes(2, "little") + bytes([len(name)]) + name
    )
...
def bprel_local(offset, typind, name):
    return record(
        0x0200,
        offset.to_bytes(4, "little", signed=True)
...
def test_proves_the_three_closed_kinds():
    detail = debug_algorithms.require_debug_symbol_representation_delta(
...
def test_refuses_an_undeclared_difference():
    with pytest.raises(ByteIdentityError, match="without a declaration"):
...
def test_refuses_a_dead_declaration():
    with pytest.raises(ByteIdentityError, match="does not differ"):
        debug_algorithms.require_debug_symbol_representation_delta(
...
def test_refuses_an_extent_off_the_length_pins():
    with pytest.raises(ByteIdentityError, match="length pins"):
        debug_algorithms.require_debug_symbol_representation_delta(
...
def test_refuses_a_debug_range_outside_its_extent():
    ...
    with pytest.raises(ByteIdentityError, match="inside its own extent"):
        debug_algorithms.require_debug_symbol_representation_delta(
...
def test_refuses_a_non_serial_label_pair():
    named = gproc(105, 95) + label(b"$T7") + DONOR[len(gproc(105, 95)) + len(label(b"$L2")) :]
    with pytest.raises(ByteIdentityError, match="compiler-numbered"):
        debug_algorithms.require_debug_symbol_representation_delta(
...
def test_refuses_a_location_off_its_declaration():
    moved = DONOR.replace(bprel_local(-8, 0x74, b"x"), bprel_local(-12, 0x74, b"x"))
    with pytest.raises(ByteIdentityError, match="location differs"):
        debug_algorithms.require_debug_symbol_representation_delta(
...
def test_refuses_a_record_count_change():
    with pytest.raises(ByteIdentityError, match="record counts differ"):
        debug_algorithms.require_debug_symbol_representation_delta(
...
def test_validator_pins_the_declaration_shape():
    normalized = debug_algorithms.validate_debug_representation_delta(DELTA, "test")
    assert normalized[2]["seed_location"] == {"register": 17}
    unsorted = [DELTA[1], DELTA[0]]
    with pytest.raises(ByteIdentityError, match="unsorted"):
        debug_algorithms.validate_debug_representation_delta(unsorted, "test")
    compiler_named = [dict(DELTA[2], name="$L80981")]
    with pytest.raises(ByteIdentityError, match="source-named"):
        debug_algorithms.validate_debug_representation_delta(compiler_named, "test")
    fixed_point = [dict(DELTA[2], donor_location={"register": 17})]
    with pytest.raises(ByteIdentityError, match="move the local"):
        debug_algorithms.validate_debug_representation_delta(fixed_point, "test")
...
def test_pins_an_inserted_donor_local():
    donor = (
        gproc(105, 95)
        + label(b"$L2")
        + bprel_local(-8, 0x74, b"x")
        + reg_local(0x75, 18, b"col")
        + END
    )
    delta = [
        *DELTA,
    ...
    with pytest.raises(ByteIdentityError, match="differs from"):
        debug_algorithms.require_debug_symbol_representation_delta(
