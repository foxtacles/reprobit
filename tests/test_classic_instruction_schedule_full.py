...
EPILOGUE = bytes.fromhex("85c0740233c05e5bc3")
...
def codeview_stream(size=SIZE, debug_start=2, debug_end=None):
    ...
    proc = bytearray(33)
    struct.pack_into(
        "<III", proc, 12, size, debug_start, size - 5 if debug_end is None else debug_end
    )
    name = b"Fixture::Read"
...
def make_coff(
    *,
    ...
    label_base = 25
    label_index = {}
    for _position, (_, label, _) in enumerate(code_relocations):
    ...
    reloc_rows += [(offset, label_index[label], 0x0006) for offset, label, _ in code_relocations]
    reloc_rows.sort()
    section_inputs = [
        {
            "name": ".text",
    ...
    assert all(label_index[label] == label_base + position for position, label in enumerate(seen))
    string_offsets = {}
...
def window_declaration(
    start=WINDOW[0], end=WINDOW[1], order=None, edges=None, line_rows=None, lengths=None
):
    return {
        "start": start,
        "end": end,
        "source_instruction_lengths": list(LENGTHS if lengths is None else lengths),
        "target_order": list(ORDER if order is None else order),
        "expected_dependence_edges": [] if edges is None else edges,
        "expected_line_rows": ([[8, 12, 3]] if line_rows is None else line_rows),
    }
...
def schedule_spec(**overrides):
    spec = {
        ...
        "authenticity_rationale": "A topological reordering of one closed call-free window whose "
...
def function_record(seed_bytes, donor_bytes, image, **overrides):
    ...
    record = {
        ...
        "splice_class": schedule_algorithms.INSTRUCTION_SCHEDULE_CLASS,
...
class DisambiguationTests(unittest.TestCase):
    ...
    def test_two_pushes_can_never_be_reordered(self):
        ...
        decoded = [
            register_algorithms.decode_ia32_bijection_instruction(
    ...
    def test_a_push_transposes_with_an_esp_free_instruction(self):
        ...
        raw = bytes.fromhex("8d8e2042000050")
        decoded = [
    ...
    def test_flags_are_a_dependence(self):
        ...
        decoded = [
            register_algorithms.decode_ia32_bijection_instruction(
                bytes.fromhex("85c0") + bytes.fromhex("1b442410"), offset, "decode"
            )
            for offset in (0, 2)
        ]
        _, edges = schedule_algorithms.ia32_schedule_dependence_edges(decoded, "dag")
...
class WindowImageTests(unittest.TestCase):
    ...
    def test_the_multiset_is_preserved(self):
        image, _ = self.apply()
        self.assertEqual(sorted(image[WINDOW[0] : WINDOW[1]]), sorted(BODY[WINDOW[0] : WINDOW[1]]))
    ...
    def test_a_call_inside_the_window_is_refused(self):
        body = PROLOGUE + bytes.fromhex("e80f000000") + WINDOW_SOURCE[4:] + EPILOGUE
        windows = [window_declaration(end=20, lengths=[5, 4, 3, 4], line_rows=[[9, 12, 3]])]
        with self.assertRaises(ByteIdentityError) as caught:
            self.apply(body, windows)
        self.assertIn("outside the instruction-schedule table", str(caught.exception))

    def test_an_x87_form_inside_the_window_is_refused(self):
        body = PROLOGUE + bytes.fromhex("d9442418") + WINDOW_SOURCE[4:] + EPILOGUE
        with self.assertRaises(ByteIdentityError) as caught:
            self.apply(body)
        self.assertIn("outside the instruction-schedule table", str(caught.exception))

    def test_a_segment_override_cannot_supply_a_disjointness_proof(self):
        # On classic Win32, FS commonly has base 0x7ffde000.  These stores can
        # therefore name the same linear address even though their encoded
        # displacements do not overlap.  Reordering them changes the final
        # value from ECX to EAX, so segment-blind alias reasoning must refuse.
        body = bytes.fromhex("64898600000000898e00e0fd7fc3")
        windows = [
            {
                "start": 0,
                "end": 13,
                "source_instruction_lengths": [7, 6],
                "target_order": [1, 0],
    ...
    def test_a_push_beside_an_esp_relative_operand_is_refused(self):
        ...
        body = PROLOGUE + bytes.fromhex("50") + WINDOW_SOURCE[4:] + EPILOGUE
        windows = [window_declaration(end=12, lengths=[1, 3, 4], order=[1, 2, 0], line_rows=[])]
        with self.assertRaises(ByteIdentityError) as caught:
            self.apply(body, windows)
    ...
    def test_a_pop_is_still_outside_the_table(self):
        ...
        body = PROLOGUE + bytes.fromhex("58") + WINDOW_SOURCE[4:] + EPILOGUE
        windows = [window_declaration(end=12, lengths=[1, 3, 4], order=[1, 2, 0], line_rows=[])]
        with self.assertRaises(ByteIdentityError) as caught:
            self.apply(body, windows)
        self.assertIn("outside the instruction-schedule table", str(caught.exception))
    ...
    def test_an_indirect_jump_makes_the_entry_set_unknowable(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            self.apply(body)
        self.assertIn("makes the window's entry set unknowable", str(caught.exception))
    ...
    def test_a_declared_edge_set_that_differs_is_refused(self):
        ...
        self.assertIn("dependence DAG differs from its declaration", str(caught.exception))
...
class DebugFidelityTests(unittest.TestCase):
    ...
    def test_a_line_row_left_off_a_boundary_is_refused(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            self.check(
                seed, IMAGE, schedule_spec(windows=[window_declaration(line_rows=[[15, 12, 2]])])
            )
        self.assertIn("is not an instruction boundary of the image", str(caught.exception))

    def test_an_undeclared_interior_line_row_is_refused(self):
        with self.assertRaises(ByteIdentityError) as caught:
            self.check(
                make_coff(), IMAGE, schedule_spec(windows=[window_declaration(line_rows=[])])
            )
    ...
    def test_a_procedure_range_inside_a_window_is_refused(self):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            self.check(seed, IMAGE, schedule_spec(expected_procedure_range=[SIZE, 8, SIZE - 5]))
        self.assertIn("falls inside a reordered window", str(caught.exception))
...
class ScheduleSchemaTests(unittest.TestCase):
        ...
        with self.assertRaises(ByteIdentityError) as caught:
            self.validate(
                windows=[
                    window_declaration(),
                    window_declaration(
                        start=8, end=19, lengths=[4, 3, 4], order=[1, 2, 0], line_rows=[]
                    ),
                ]
            )
    ...
    def test_an_unknown_dependence_reason_is_refused(self):
        with self.assertRaises(ByteIdentityError):
            self.validate(windows=[window_declaration(edges=[[0, 1, ["handwave"]]])])
...
class ScheduleCompositionTests(unittest.TestCase):
    """Obligations 1, 8 and 9: oracle-free candidate production."""
    ...
    def test_the_certificate_produces_the_declared_candidate(self):
        composed, detail = schedule_algorithms.produce_instruction_schedule_candidate(
        ...
        self.assertEqual(detail["debug_fidelity"]["window_line_rows"], [[8, 12, 3]])

    def test_candidate_api_refuses_a_literal_oracle_argument(self):
        oracle = bytearray(IMAGE)
        oracle[WINDOW[1]] ^= 0x01
        with self.assertRaises(TypeError):
            schedule_algorithms.produce_instruction_schedule_candidate(
    ...
    def test_a_body_that_is_already_the_oracle_still_needs_the_proof(self):
        ...
        record["expected_body_sha256"] = foundation_algorithms.sha256_bytes(BODY)
        with self.assertRaises(ByteIdentityError) as caught:
            schedule_algorithms.produce_instruction_schedule_candidate(
    ...
    def test_the_output_keeps_the_seed_line_and_relocation_tables(self):
        composed, _ = schedule_algorithms.produce_instruction_schedule_candidate(
            self.seed, self.donor, self.record
        )
    ...
    def test_the_delegate_is_read_off_the_closure_pin(self):
        self.assertEqual(
            schedule_algorithms.instruction_schedule_delegate([".debug$F", ".debug$S"], []),
            "equal_body_strict",
        )
        self.assertEqual(
            schedule_algorithms.instruction_schedule_delegate([".debug$S", ".xdata$x"], []),
...
class SwitchTableTailTests(unittest.TestCase):
    ...
    def test_the_pinned_code_decodes_and_the_tail_does_not(self):
        ...
        self.assertEqual(sum(length for _, length in spans), SWITCH_CODE_LENGTH)
    ...
    def test_a_relocated_target_off_an_instruction_boundary_is_refused(self):
        ...
        self.assertIn("is not an instruction boundary of this body", str(caught.exception))
    ...
    def test_the_unpinned_walk_decodes_the_table_as_code(self):
        ...
        self.assertEqual(sum(length for _, length in spans), len(SWITCH_BODY))

    def test_a_window_that_reaches_into_the_tail_is_refused(self):
        windows = [
            window_declaration(
                end=SWITCH_CODE_LENGTH + 4,
                lengths=[*LENGTHS, 7, 4],
                order=[1, 3, 0, 2, 4, 5],
                line_rows=[],
            )
...
def switch_spec(**overrides):
    spec = schedule_spec(
        ...
        expected_changed_offsets=sorted(
            index for index in range(len(SWITCH_BODY)) if SWITCH_BODY[index] != SWITCH_IMAGE[index]
        ),
...
class SwitchTableSchemaTests(unittest.TestCase):
    ...
    def test_the_switch_declaration_validates(self):
        normalized = self.validate()
        self.assertEqual(normalized["expected_code_length"], SWITCH_CODE_LENGTH)
        self.assertEqual(normalized["expected_internal_relocation_targets"], sorted(SWITCH_TARGETS))

    def test_a_window_past_the_declared_code_length_is_refused(self):
        with self.assertRaises(ByteIdentityError) as caught:
            self.validate(expected_code_length=12)
        self.assertIn("reaches past the declared code length", str(caught.exception))
...
class SwitchTableCompositionTests(unittest.TestCase):
    ...
    def setUp(self):
        stream = codeview_stream(size=len(SWITCH_BODY), debug_start=2, debug_end=19)
        self.seed = make_coff(
            body=SWITCH_BODY, debug_stream=stream, code_relocations=SWITCH_RELOCATIONS
        )
        self.donor = make_coff(
            body=SWITCH_BODY, debug_stream=stream, code_relocations=SWITCH_RELOCATIONS
        )
        coff = coff_format.CoffObject(self.seed)
        rows = coff_format.detailed_relocations(coff, coff.function_section(TARGET_SYMBOL))
        oracle = []
        for row in rows:
            raw = int.from_bytes(SWITCH_IMAGE[row["offset"] : row["offset"] + 4], "little")
            oracle.append(
                {
                    field: row[field]
                    for field in (
            ...
            oracle[-1]["retail_target"] = "0x%08x" % ((raw - row["addend"]) & 0xFFFFFFFF)
    ...
    def test_the_certificate_produces_the_declared_candidate(self):
        ...
        coff = coff_format.CoffObject(composed)
        section = coff.function_section(TARGET_SYMBOL)
        self.assertEqual(coff_format.coff_body(coff, section), SWITCH_IMAGE)
        self.assertTrue(detail["candidate_only"])
    ...
    def test_a_target_set_that_differs_from_its_pin_is_refused(self):
        record = copy.deepcopy(self.record)
        record["instruction_schedule"]["expected_internal_relocation_targets"] = [0, 19]
        with self.assertRaises(ByteIdentityError) as caught:
            schedule_algorithms.produce_instruction_schedule_candidate(
                self.seed, self.donor, record
            )
        self.assertIn("in-body relocated target set changed", str(caught.exception))
