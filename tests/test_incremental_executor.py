...
def test_dependents_start_before_unrelated_slow_siblings_finish(tmp_path: Path) -> None:
    """A node runs as soon as its own dependencies finish, not at a wave end."""
    ...
    def slow(_runtime: object, _cancellation: object, _inputs: PreparedNodeInputs) -> None:
        if not slow_may_finish.wait(timeout=5):
            raise RuntimeError("child never ran while its unrelated sibling was busy")
        (workspace / "slow.obj").write_bytes(b"slow")

    def fast(_runtime: object, _cancellation: object, _inputs: PreparedNodeInputs) -> None:
        (workspace / "fast.obj").write_bytes(b"fast")

    def child(_runtime: object, _cancellation: object, inputs: PreparedNodeInputs) -> None:
        payload = inputs.entries["build/fast.obj"].snapshot.path.read_bytes()
        (workspace / "child.obj").write_bytes(payload + b"+child")
        child_ran.set()
        slow_may_finish.set()

    progress: list[tuple[ProgressKind, int, int, str]] = []
    result = IncrementalDAGExecutor(
        cache=IncrementalCache(state, implementation="dag-test-v1"),
        workspace_root=workspace,
        runtime_factory=object,
        runtime_close=lambda _runtime: None,
        max_workers=2,
        progress=lambda kind, done, total, _phase, node, _reason: progress.append(
            (kind, done, total, node)
        ),
    ).execute(
        (
            IncrementalNode(
                ...
                domain="producer",
                depends_on=(),
                outputs={"build/slow.obj": workspace / "slow.obj"},
                key=lambda _deps: _node_key("eager.slow"),
                execute=slow,
                metadata=lambda _deps: {},
            ),
            IncrementalNode(
                ...
                domain="producer",
                depends_on=(),
                outputs={"build/fast.obj": workspace / "fast.obj"},
                key=lambda _deps: _node_key("eager.fast"),
                execute=fast,
                metadata=lambda _deps: {},
            ),
            IncrementalNode(
                ...
                domain="producer",
                depends_on=("fast",),
                outputs={"build/child.obj": workspace / "child.obj"},
                key=lambda deps: _node_key("eager.child", deps["fast"].record.outputs[0].digest),
                execute=child,
                materialize_inputs=_materialize_inputs(
                    workspace, "child", {"build/fast.obj": "fast"}
                ),
                metadata=lambda _deps: {},
            ),
        )
    ...
    assert (workspace / "child.obj").read_bytes() == b"fast+child"
    # "child" releases "slow" before its own record is staged, so the two may
    # finish in either order; what the scheduler guarantees is that "child"
    # ran at all while "slow" was still blocked, and that "fast" led.
    finished = [item[3] for item in progress if item[0] == ProgressKind.UNIT_FINISHED]
    assert finished[0] == "fast"
    assert sorted(finished[1:3]) == ["child", "slow"]
    assert finished[3] == "cache-record-set"
    assert [item[1:3] for item in progress if item[0] == ProgressKind.UNIT_FINISHED] == [
        (1, 4),
        (2, 4),
        (3, 4),
        (4, 4),
...
def test_nodes_behind_a_dependency_cycle_are_reported_before_any_node_runs(
    tmp_path: Path,
) -> None:
    state = tmp_path / "state"
    state.mkdir()
    ...
    def action(name: str) -> Callable[[object, object, PreparedNodeInputs], None]:
        def execute(_runtime: object, _cancellation: object, _inputs: PreparedNodeInputs) -> None:
            executed.append(name)
            (workspace / f"{name}.obj").write_bytes(name.encode())

        return execute

    def node(name: str, *dependencies: str) -> IncrementalNode[object]:
        return IncrementalNode(
            id=name,
            domain="producer",
            depends_on=dependencies,
            outputs={f"build/{name}.obj": workspace / f"{name}.obj"},
            key=lambda _deps, current=name: _node_key(f"cycle.{current}"),
            execute=action(name),
            metadata=lambda _deps: {},
            order_only=tuple(sorted(dependencies, key=str.casefold)),
        )
    ...
    with pytest.raises(
        IncrementalExecutionError,
        match=r"cannot make progress: \{'first': \['second'\], 'second': \['first'\], "
        r"'third': \['second'\]\}",
    ):
        executor.execute(
            (
                ...
                node("first", "second"),
                node("second", "first"),
                node("third", "second", "free"),
            )
        )
    assert executed == []
    assert not (workspace / "free.obj").exists()
