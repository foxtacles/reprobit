...
def _summary(result: ProjectGrindResult) -> tuple[str, str, str, tuple[str, ...]]:
    if result.published:
        return (
            "ok",
            "Exact intervention saved",
            "The chosen compiler state passed a fresh cold build, matched every target "
            "byte for byte, and passed its required logic checks. ReproBit then saved the "
            "intervention and proof records together.",
            (
                _claim("Exact match", tone="ok"),
                _claim("Cold verified", tone="ok"),
    ...
    if result.exact:
        return (
            "ok",
            "Exact intervention ready for review",
            "The chosen compiler state passed a fresh cold build, matched every target "
            "byte for byte, and passed its required logic checks. This preview wrote only "
            "review reports; the project files stayed unchanged.",
            (
                _claim("Exact match", tone="ok"),
                _claim("Cold verified", tone="ok"),
    ...
    return (
        "warn",
        "No exact intervention within these bounds",
...
def _funnel(result: ProjectGrindResult) -> str:
    bars = (
        ...
        Bar(
            label="Cold trials",
            detail="Received a fresh verification build",
    ...
    return bar_chart(
        ...
        title="Search funnel",
        description="How the bounded states narrowed to an admitted exact result.",
        bars=bars,
    )
...
def _rejection_chart(result: ProjectGrindResult) -> str:
    def friendly_reason(rejection: GrindRejection) -> str:
        ...
        return "Cold verification did not pass"
    ...
    bars = tuple(
        Bar(
            label=reason,
            detail=(
                f"{stage_counts[(reason, 'qualification')]} qualification · "
                f"{stage_counts[(reason, 'cold_verification')]} cold"
            ),
            value=float(count),
    ...
    return bar_chart(
        ...
        description="Quick-check failures and cold-build rejections by reason.",
        bars=bars,
...
<article class="card decision-card warn">
  ...
  <p>Review the rejection chart before widening the bounded plan.</p>
</article>"""
    ...
    if solution.reused_donor:
        authority_change = (
            "One function intervention was saved; the matching shared donor was reused "
            "and its beneficiary attribution was updated."
            if result.published
            else "Approval will save one function intervention, reuse the matching shared "
            "donor, and update its beneficiary attribution."
        )
    else:
        authority_change = (
            "Two intervention records and their matching proof records were saved."
            if result.published
            else "Approval will save two intervention records and their matching proof records."
        )
    if result.published:
        publish_status = '<div class="status">Project files updated</div>'
        publish_detail = (
            "Saved together in transaction "
            f"<code>{escape(result.transaction_id or 'not recorded')}</code>"
        )
        files_label = "Changed files"
        next_commands = """
  <p><strong>Review the saved authority, then run a separate cold verification:</strong></p>
  <pre><code>git diff
rbit verify .</code></pre>"""
    else:
  ...
  <pre><code>{escape(approval_command)}</code></pre>"""
    verification = ""
    if cold_report_html is not None:
        verification = f"""
  <a class="machine-link" href="{escape(cold_report_html)}">Open cold verification report</a>
...
def _technical_details(
    ...
    *,
    ...
    body = (
        f'<dl class="identity-list">{definitions}</dl>'
        + table(
            ("State", "Stage", "Reason"),
            rejection_rows,
    ...
    *,
    ...
    metrics = "".join(
        (
            ...
            "<p>seed plus donor states</p></article>",
            f'<article class="card"><h3>Cold trials</h3><div class="value">'
...
<header class="topbar"><div class="topbar-inner">
  <div class="brand">ReproBit grind · <code>{escape(result.project_id)}</code></div>
...
<section class="hero {escape(tone)}" id="overview" aria-labelledby="report-title">
  <p class="eyebrow">Automatic intervention grind</p>
  <h1 id="report-title">{escape(heading)}</h1>
  <p class="lede">{escape(explanation)}</p>
  <div class="claim-row" aria-label="Grind outcome">{''.join(claims)}</div>
...
<section class="section" id="decision" aria-labelledby="decision-title">
  <p class="eyebrow">Final decision</p><h2 id="decision-title">Chosen state and project changes</h2>
  {_decision(
      result,
