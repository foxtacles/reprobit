...
_FORWARD_PREFIX = re.compile(r"[A-Za-z][A-Za-z0-9]*", re.ASCII)
_EXTERN_PREFIX = re.compile(r"[A-Za-z][A-Za-z0-9_]*", re.ASCII)
...
def generate_forward_run(prefix: str, count: int, width: int) -> str:
    ...
    """
    if _FORWARD_PREFIX.fullmatch(prefix) is None:
...
def generate_extern_run(prefix: str, count: int, width: int) -> str:
    ...
    """
    if _EXTERN_PREFIX.fullmatch(prefix) is None:
