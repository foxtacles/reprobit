...
def _add_compiler_node(
    ...
    *,
    ...
    sampled_inputs: set[Path],
    overlay_inputs: tuple[JsonValue, ...],
    generated_inputs: tuple[JsonValue, ...],
) -> None:
                ...
                sorted(
                    {
                        ...
                        *recursive_sampled_paths(tuple(item.logical_path for item in probe.reads)),
                    },
                    key=str,
                )
            )
    ...
    def compiler_action(
        ...
        *,
        ...
        runtime.prepared.warm.execute_warm_graph_node(
            current_node.id,
            inputs=prepared_inputs,
            outputs=current_outputs,
            cancellation=cancellation,
        )
        replay = runtime.prepared.warm.replay_warm_compiler_dependencies(
            current_node.id,
            cancellation=cancellation,
        )
            ...
            try:
                ...
                current_state.physical_inputs = tuple(
                    sorted(
                        {
                            *current_state.physical_inputs,
                            *recursive_sampled_paths(tuple(item.logical_path for item in reads)),
                        },
                        key=str,
                    )
                )
        ...
        if current_state.replay_failure is not None:
            if (
                ...
                interventions = authority.protected_sources[current_source_relative.casefold()]
    ...
    def compiler_final_key_factory(
        ...
        *,
        ...
        if current_state.final_key is None:
            raise ClassicIncrementalError("compiler action omitted its final dependency key")
    ...
    def compiler_key_factory(
        dependencies: Mapping[str, NodeOutcome],
        *,
        current: Callable[[Mapping[str, NodeOutcome]], dict[str, JsonValue]] = compiler_material,
    ) -> str:
...
def _add_ordinary_producer_node(
    plan: ClassicIncrementalPlan,
    node: ProducerNode,
    *,
    graph_dependencies: tuple[str, ...],
    ...
    if node.role is ProducerRole.RESOURCE:
        ...
        sampled_inputs.update(
            recursive_sampled_paths(tuple(item.logical_path for item in resource_receipt.reads))
        )
    ...
    if node.role is ProducerRole.LINKER:
        ...
        link_references = tuple(reference for group in link_reference_groups for reference in group)
    ...
    def producer_key_factory(
        dependencies: Mapping[str, NodeOutcome],
        *,
        current: Callable[[Mapping[str, NodeOutcome]], dict[str, JsonValue]] = producer_material,
    ) -> str:
...
def add_producer_nodes(plan: ClassicIncrementalPlan) -> None:
    for node in plan.graph.nodes:
        generated = node.id in plan.generated_nodes
        graph_dependencies = tuple(
            sorted(
                (plan.transform_ids.get(dependency, dependency) for dependency in node.depends_on),
                key=str.casefold,
            )
        )
        order_only: tuple[str, ...] = ()
        if generated:
            added = tuple(
                dependency
                for dependency in plan.ordinary_barrier
        ...
        arguments = warm_context.node_arguments(plan, node)
        direct = warm_context.direct_inputs(plan, node, generated=generated)
        sampled_inputs = {
            path
            for reference in (*node.inputs, *node.directive_inputs)
            for path in (warm_context.sampled_reference_path(plan, reference),)
            if path is not None
        }
        role_path = plan.census.known_path(
            plan.toolchain_root.joinpath(*PurePosixPath(plan.role_relatives[node.role]).parts)
        )
        if role_path is not None:
            sampled_inputs.add(role_path)
        sampled_inputs.update(plan.runtime_input_paths)
        overlay_inputs = tuple(
            plan.overlay_by_path[path]
            for path in sorted(plan.overlay_by_path, key=str.casefold)
            if path.casefold()
            in {
        ...
        generated_inputs = tuple(
            plan.overlay_by_path[path]
            for path in sorted(plan.overlay_by_path, key=str.casefold)
            if generated and path.casefold() in plan.generated_paths
        )
...
def _add_transform_node(
    plan: ClassicIncrementalPlan,
    compiler_id: str,
    transform_unit: ClassicPreparedUnit,
    *,
    all_overlay_inputs: tuple[JsonValue, ...],
) -> None:
    authority = plan.authority
    authority_by_epoch = plan.authority_by_epoch
    bundle = plan.bundle
    ...
    def transform_key_factory(
        dependencies: Mapping[str, NodeOutcome],
        *,
        current: Callable[[Mapping[str, NodeOutcome]], dict[str, JsonValue]] = transform_material,
        current_state: TransformState | None = transform_state,
    ) -> str:
...
def add_transform_nodes(plan: ClassicIncrementalPlan) -> None:
    all_overlay_inputs = tuple(
        plan.overlay_by_path[path] for path in sorted(plan.overlay_by_path, key=str.casefold)
    )
    for compiler_id, transform_unit in plan.compiler_units.items():
        _add_transform_node(
            plan,
            compiler_id,
            transform_unit,
            all_overlay_inputs=all_overlay_inputs,
        )
