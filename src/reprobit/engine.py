...
from reprobit.verify import ComparisonReceipt, LiteralVerifier, SealedFileOracle
...
# Keep the historical public name while the implementation and proof issuer
# share one authoritative contract type.
ClassicSemanticValidatorContract = RegisteredSemanticContract


CLASSIC_SEMANTIC_VALIDATORS: Mapping[ClassicRecipeFamily, ClassicSemanticValidatorContract] = (
    CLASSIC_SEMANTIC_CONTRACTS
)
...
def _has_registered_semantic_proof(
    ...
    contract: ClassicSemanticValidatorContract,
) -> bool:
...
class EvidenceAuditor:
    ...
    def audit(
        ...
        for intervention_id in sorted(interventions):
            ...
            contract = (
                CLASSIC_SEMANTIC_VALIDATORS.get(classic_intervention.family)
...
def _resolve_builtin_identity(request: EngineRequest) -> _ExecutionIdentity:
    ...
    if getattr(provider, "_executor", None) is not executor:
...
__all__ = [
    "CLASSIC_SEMANTIC_VALIDATORS",
    "BuildExecutionReceipt",
    "BuildExecutor",
    "BuildPlanExecutor",
    "ClassicSemanticValidatorContract",
    "EngineError",
    "EngineRequest",
    "EngineResult",
    "EvidenceAudit",
