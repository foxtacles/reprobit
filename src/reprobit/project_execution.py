"""Application operations for incremental builds and cold verification."""
...
def _execute_build(
    ...
    *,
    ...
    with progress.activity("checking the project files", phase="validate"):
        if request.cold and request.repair_analysis is not None:
            # Internal repair trials use their staged, explicitly reviewed pins.
            ...
            if isinstance(committed.spec.build, ProducerGraphBuildAdapter) and (
                not request.cold
                or (committed.producer_graph is not None and committed.build_plan is not None)
            ):
                ...
                if request.cold:
                    ...
                    validate_source_authority(bundle, root, render_session=overlay_session)
            else:
