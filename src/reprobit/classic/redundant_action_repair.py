"""Plan removal of a classic function action made redundant by fresh source."""
...
class RedundantActionRetirement:
    """Exact authority edits needed to retire one now-redundant action."""
...
def plan_redundant_action_retirement(
    ...
    """Propose retiring an action only when fresh source already emits its body goal."""

    if action.role is not ClassicRecipeRole.FUNCTION or action.symbol is None:
        raise RedundantActionRepairError("redundant-action repair accepts only function recipes")
    ...
    remaining = tuple(item for item in interventions if item.id != action.id)
    remaining_receipts = tuple(item for item in receipts if item.intervention_id != action.id)
    receipt_index = _receipt_by_intervention(remaining_receipts)
    retiring_receipt_index = _receipt_by_intervention((receipt,))
    affected_donors = _consumer_donor_ids(action, retiring_receipt_index)
    ...
    intervention_edits: list[ClassicInterventionEdit] = [ClassicInterventionEdit(action, None)]
    removed_donors: list[str] = []
    removed_ids = {action.id}
    ...
    if receipt.id not in {item.before.id for item in receipt_edits}:
