...
def require_source_overlay_range_pin(
    ...
    require(
        ...
        and (actual["actual_removed_range_line_count"] == expected["baseline_line_count"])
        and (
            actual["actual_removed_range_significant_token_sha256"]
...
GENERATED_DECLARATION_LINE = re.compile(
    b"^(?:class|struct)[ ]+[A-Za-z_][A-Za-z0-9_]*[0-9]{2,};$|^extern[ ]int[ ]+[A-Za-z_][A-Za-z0-9_]*[0-9]{2,};$"
)
...
def select_source_permutation_window(data: bytes, proof: dict[str, Any], context: str) -> bytes:
    ...
    require(
        ...
        and proof["selector"] == "brace_balanced_function_after_marker_v1",
...
def require_target_source_refactor_identity(
    ...
    require(
        seed_range != donor_range, f"{context} donor does not contain its declared source refactor"
    )
    return {
