...
def associated_sections(coff: _CoffObject, definitions: dict[int, dict], parent: int) -> tuple[tuple[int, str], ...]:
...
def require_target_closure_extraction_topology(seed: _CoffObject, donor: _CoffObject, function: dict, context: str) -> dict:
...
def require_source_target_closure_topology(seed: _CoffObject, donor: _CoffObject, function: dict, context: str) -> dict:
    ...
    require(sha256_bytes(_coff_body(seed, sp)) == function['expected_seed_body_sha256'], f'{context} seed target body changed')
    ...
    for name, (seat_key, seed_sha_key, donor_sha_key) in expected.items():
        ...
        require(left['number'] == right['number'] == function[seat_key] and sha256_bytes(_coff_body(seed, left)) == function[seed_sha_key] and (sha256_bytes(_coff_body(donor, right)) == function[donor_sha_key]), f'{context} pinned {name} closure changed')
