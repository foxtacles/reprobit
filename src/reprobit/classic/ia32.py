...
def validate_retail_relocation_oracle(value: object, context: str, body_length: int, *, allow_empty: bool=False) -> list[dict]:
    """Validate an ordered COFF-to-retail semantic relocation oracle."""
    require(isinstance(value, list) and (value or allow_empty), f"{context} must be {('an array' if allow_empty else 'a non-empty array')}")
    normalized = []
    previous_end = 0
    for index, item in enumerate(value):
        ...
        offset = require_exact_int(item.get('offset'), item_context + '.offset', minimum=0, maximum=body_length - 4)
        require(offset >= previous_end, f'{context} relocation operands overlap or are unordered')
        previous_end = offset + 4
        relocation_type = require_exact_int(item.get('type'), item_context + '.type', minimum=0, maximum=65535)
        require(relocation_type in (6, 20), f'{item_context}.type is not DIR32 or REL32')
        target = item.get('target')
        require(isinstance(target, str) and target and target.isascii() and (len(target) <= 4096) and ('\x00' not in target), f'{item_context}.target is invalid')
        retail_target = item.get('retail_target')
        require(isinstance(retail_target, str) and ADDRESS_RE.fullmatch(retail_target) is not None, f'{item_context}.retail_target is invalid')
        normalized.append({'offset': offset, 'type': relocation_type, 'addend': require_exact_int(item.get('addend'), item_context + '.addend', minimum=0, maximum=4294967295), 'target': target, 'target_section': require_exact_int(item.get('target_section'), item_context + '.target_section', minimum=0, maximum=32767), 'target_value': require_exact_int(item.get('target_value'), item_context + '.target_value', minimum=0, maximum=4294967295), 'target_type': require_exact_int(item.get('target_type'), item_context + '.target_type', minimum=0, maximum=65535), 'target_storage': require_exact_int(item.get('target_storage'), item_context + '.target_storage', minimum=0, maximum=255), 'retail_target': retail_target})
    return normalized
INSTRUCTION_MOSAIC_RANGE_KEYS = {'kind', 'start', 'end', 'seed_sha256', 'donor_sha256', 'donor', 'relocation_reseat', 'seed_relocation_offsets', 'donor_relocation_offsets'}
INSTRUCTION_MOSAIC_RANGE_OPTIONAL_KEYS = {'donor', 'relocation_reseat', 'seed_relocation_offsets', 'donor_relocation_offsets'}
INSTRUCTION_MOSAIC_SEQUENCE_RANGE_KEYS = INSTRUCTION_MOSAIC_RANGE_KEYS | {'seed_instruction_lengths', 'donor_instruction_lengths'}
...
SELF_PERMUTATION_KINDS = frozenset({ORDINARY_FPO_SELF_PERMUTATION_KIND, INDEPENDENT_PAIR_SELF_PERMUTATION_KIND})
...
def require_coff_line_certified_ia32_boundaries(coff, section: dict, body: bytes, ranges: list[dict], role: str, mangled: str, context: str) -> int:
    ...
    for index, item in enumerate(ranges):
        ...
        while cursor < end:
            ...
            decoded_count += 1
        require(start in boundaries and end in boundaries, f'{context}: range {index} is not one containing-stream IA-32 instruction boundary')
        lengths_key = f'{role}_instruction_lengths'
        if lengths_key in item:
            require(in_range_lengths == item[lengths_key], f'{context}: range {index} decoded instruction partition differs from {lengths_key}')
    return decoded_count

def validate_cross_tu_instruction_hybrid_ranges(value: object, context: str, target_body_length: int, instruction_donor_body_length: int, *, range_kind: str='cross_tu_same_mangled_complete_x86_instruction_v1', require_same_offsets: bool=False) -> list[dict]:
    ...
    """
    require(isinstance(value, list) and 1 <= len(value) <= 64, f'{context} must contain 1..64 instruction ranges')
    normalized = []
    ...
    for index, item in enumerate(value):
        ...
        target_start = require_exact_int(item.get('target_start'), item_context + '.target_start', minimum=0, maximum=target_body_length - 1)
        target_end = require_exact_int(item.get('target_end'), item_context + '.target_end', minimum=1, maximum=target_body_length)
        source_start = require_exact_int(item.get('instruction_donor_start'), item_context + '.instruction_donor_start', minimum=0, maximum=instruction_donor_body_length - 1)
        source_end = require_exact_int(item.get('instruction_donor_end'), item_context + '.instruction_donor_end', minimum=1, maximum=instruction_donor_body_length)
        target_width = target_end - target_start
        source_width = source_end - source_start
        require(1 <= target_width == source_width <= 15, f'{item_context} is not one equal-width x86 instruction')
        require(target_start >= previous_target_end and source_start >= previous_source_end, f'{context} is unsorted or overlapping')
        if require_same_offsets:
            require((target_start, target_end) == (source_start, source_end), f'{item_context}: source-aware hybrid offsets differ')
        ...
        require(role_values['target'] != role_values['instruction_donor'], f'{item_context} does not change compiler output')
        normalized.append({'kind': item['kind'], 'target_start': target_start, 'target_end': target_end, 'target_sha256': role_values['target'], 'instruction_donor_start': source_start, 'instruction_donor_end': source_end, 'instruction_donor_sha256': role_values['instruction_donor']})
    return normalized
...
def decode_commutable_ia32_instruction(encoded: bytes, context: str) -> dict:
    ...
    """
    require(isinstance(encoded, (bytes, bytearray)) and len(encoded) >= 2, f'{context}: commutable instruction is too short')
...
def require_commuting_ia32_instruction_pair(first: bytes, second: bytes, context: str) -> dict:
    ...
    """
    decoded = [decode_commutable_ia32_instruction(first, f'{context} first'), decode_commutable_ia32_instruction(second, f'{context} second')]
    require(not any((item['writes_memory'] for item in decoded)), f'{context}: a commuting instruction writes memory')
    require(not decoded[0]['writes'] & decoded[1]['writes'], f'{context}: commuting instructions write the same location')
    require(not decoded[0]['reads'] & decoded[1]['writes'] and (not decoded[1]['reads'] & decoded[0]['writes']), f'{context}: commuting instructions carry a register dependency')
    return {'first_reads': sorted(decoded[0]['reads']), 'first_writes': sorted(decoded[0]['writes']), 'second_reads': sorted(decoded[1]['reads']), 'second_writes': sorted(decoded[1]['writes'])}
...
def validate_instruction_self_permutation(value: object, context: str, body: bytes) -> dict:
    ...
    exact_audit_keys(value, {'kind', 'source_start', 'source_end', 'target_start', 'target_end', 'source_instruction_lengths', 'target_instruction_lengths', 'moves', 'expected_changed_offsets', 'expected_function_multiset_sha256', 'expected_comdat_multiset_sha256', 'expected_section_shape_sha256', 'expected_linker_payload_count', 'expected_linker_payload_sha256'}, context)
    require(value.get('kind') in SELF_PERMUTATION_KINDS, f'{context}.kind differs')
    bounds = {}
    for role in ('source', 'target'):
        start = require_exact_int(value.get(f'{role}_start'), f'{context}.{role}_start', minimum=0, maximum=body_length - 1)
        end = require_exact_int(value.get(f'{role}_end'), f'{context}.{role}_end', minimum=1, maximum=body_length)
    ...
    for role in ('source', 'target'):
        ...
        require(isinstance(lengths, list) and len(lengths) == 2 and all((type(length) is int and 1 <= length <= 15 for length in lengths)) and (sum(lengths) == width), f'{context}.{role}_instruction_lengths differs')
        declared_lengths[role] = list(lengths)
    moves = value.get('moves')
    require(isinstance(moves, list) and len(moves) == 2, f'{context}.moves must contain exactly two instructions')
    normalized_moves = []
    for index, move in enumerate(moves):
        move_context = f'{context}.moves[{index}]'
        require(isinstance(move, dict), f'{move_context} must be an object')
        exact_audit_keys(move, {'target_start', 'target_end', 'target_sha256', 'donor_start', 'donor_end', 'donor_sha256'}, move_context)
        intervals = {}
        for role, prefix in (('target', 'target'), ('source', 'donor')):
            outer_start, outer_end = bounds[role]
            start = require_exact_int(move.get(f'{prefix}_start'), f'{move_context}.{prefix}_start', minimum=outer_start, maximum=outer_end - 1)
            end = require_exact_int(move.get(f'{prefix}_end'), f'{move_context}.{prefix}_end', minimum=outer_start + 1, maximum=outer_end)
            require(start < end, f'{move_context}.{prefix} interval is empty')
            intervals[role] = (start, end)
        require(intervals['target'][1] - intervals['target'][0] == intervals['source'][1] - intervals['source'][0], f'{move_context} source/target widths differ')
        ...
        require(target_sha == donor_sha, f'{move_context} does not copy one exact donor instruction')
        donor_raw = body[intervals['source'][0]:intervals['source'][1]]
        require(sha256_bytes(donor_raw) == donor_sha, f'{move_context} differs from the fresh donor artifact')
        require_supported_complete_ia32_instruction(donor_raw, f'{move_context}.donor')
        normalized_moves.append({'target_start': intervals['target'][0], 'target_end': intervals['target'][1], 'target_sha256': target_sha, 'donor_start': intervals['source'][0], 'donor_end': intervals['source'][1], 'donor_sha256': donor_sha})
    for role, start_key, end_key in (('target', 'target_start', 'target_end'), ('source', 'donor_start', 'donor_end')):
        ordered = sorted(normalized_moves, key=lambda item: item[start_key])
        cursor = bounds[role][0]
        lengths = []
        for move in ordered:
            require(move[start_key] == cursor, f'{context}: {role} partition is overlapping or gapped')
            cursor = move[end_key]
            lengths.append(move[end_key] - move[start_key])
        require(cursor == bounds[role][1] and lengths == declared_lengths[role], f'{context}: {role} partition differs from its declaration')
    require([(item['donor_start'], item['donor_end']) for item in normalized_moves] == list(reversed([(item['donor_start'], item['donor_end']) for item in sorted(normalized_moves, key=lambda item: item['donor_start'])])), f'{context}: target order is not the exact two-instruction reversal')
    ...
    if value['kind'] == ORDINARY_FPO_SELF_PERMUTATION_KIND:
        require(first == b'3\xff', f'{context}: first source instruction is not the closed XOR-zero encoding')
        zero_modrm = first[1]
        zero_reg = zero_modrm >> 3 & 7
        require(zero_modrm >> 6 == 3 and zero_modrm & 7 == zero_reg, f'{context}: XOR does not zero exactly one register')
        require(second == b'\x8bT$\x14', f'{context}: second source instruction is not the closed stack-load encoding')
    independence = require_commuting_ia32_instruction_pair(first, second, f'{context} commuting pair')
    normalized = {'kind': value['kind'], 'commuting_pair_independence': independence, 'source_start': bounds['source'][0], 'source_end': bounds['source'][1], 'target_start': bounds['target'][0], 'target_end': bounds['target'][1], 'source_instruction_lengths': declared_lengths['source'], 'target_instruction_lengths': declared_lengths['target'], 'moves': normalized_moves, 'expected_changed_offsets': [], 'expected_linker_payload_count': require_exact_int(value.get('expected_linker_payload_count'), f'{context}.expected_linker_payload_count', minimum=0)}
    changed_offsets = value.get('expected_changed_offsets')
    require(isinstance(changed_offsets, list) and changed_offsets and (changed_offsets == sorted(set(changed_offsets))) and all((type(offset) is int and 0 <= offset < body_length for offset in changed_offsets)), f'{context}.expected_changed_offsets differs')
    normalized['expected_changed_offsets'] = list(changed_offsets)
    for name in ('expected_function_multiset_sha256', 'expected_comdat_multiset_sha256', 'expected_section_shape_sha256', 'expected_linker_payload_sha256'):
...
def validate_instruction_mosaic_ranges(value: object, context: str, body_length: int) -> list[dict]:
    ...
    for index, item in enumerate(value):
        ...
        if kind == 'same_offset_complete_x86_instruction_v1':
            exact_audit_keys(item, INSTRUCTION_MOSAIC_RANGE_KEYS, item_context, optional=INSTRUCTION_MOSAIC_RANGE_OPTIONAL_KEYS)
        elif kind == 'same_offset_complete_x86_instruction_sequence_v1':
            exact_audit_keys(item, INSTRUCTION_MOSAIC_SEQUENCE_RANGE_KEYS, item_context, optional=INSTRUCTION_MOSAIC_RANGE_OPTIONAL_KEYS)
        else:
            raise ByteIdentityError(f'{item_context}.kind differs')
        start = require_exact_int(item.get('start'), item_context + '.start', minimum=0, maximum=body_length - 1)
        end = require_exact_int(item.get('end'), item_context + '.end', minimum=1, maximum=body_length)
        maximum = 15 if kind == 'same_offset_complete_x86_instruction_v1' else 64
        require(start < end and end - start <= maximum, f'{item_context} is not a bounded x86 instruction range')
        ...
        if reseat_keys & set(item):
            require(item.get('relocation_reseat') is True and reseat_keys <= set(item), f'{item_context} relocation reseat declaration is incomplete')
            offsets = {}
            for role in ('seed', 'donor'):
                values = item.get(f'{role}_relocation_offsets')
                require(isinstance(values, list) and 1 <= len(values) <= 16 and all((type(v) is int and start <= v and (v + 4 <= end) for v in values)) and (values == sorted(set(values))), f'{item_context}.{role}_relocation_offsets differ')
                offsets[role] = list(values)
            require(len(offsets['seed']) == len(offsets['donor']) and offsets['seed'] != offsets['donor'], f'{item_context} relocation reseat does not move a relocation operand')
        ...
        if kind == 'same_offset_complete_x86_instruction_sequence_v1':
            for role in ('seed', 'donor'):
                ...
                require(isinstance(lengths, list) and lengths and (len(lengths) <= 64) and all((type(length) is int and 1 <= length <= 15 for length in lengths)) and (sum(lengths) == end - start), f'{item_context}.{role}_instruction_lengths differ')
...
def require_declared_relocation_semantics(
    ...
    fields = ('offset', 'type', 'addend', 'target', 'target_section', 'target_value', 'target_type', 'target_storage')
