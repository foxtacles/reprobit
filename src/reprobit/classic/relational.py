...
IA32_CONDITION_FLAGS = {0: frozenset({'of'}), 1: frozenset({'of'}), 2: frozenset({'cf'}), 3: frozenset({'cf'}), 4: frozenset({'zf'}), 5: frozenset({'zf'}), 6: frozenset({'cf', 'zf'}), 7: frozenset({'cf', 'zf'}), 8: frozenset({'sf'}), 9: frozenset({'sf'}), 10: frozenset({'pf'}), 11: frozenset({'pf'}), 12: frozenset({'sf', 'of'}), 13: frozenset({'sf', 'of'}), 14: frozenset({'zf', 'sf', 'of'}), 15: frozenset({'zf', 'sf', 'of'})}
IA32_CONDITION_NAMES = {0: 'o', 1: 'no', 2: 'b', 3: 'ae', 4: 'e', 5: 'ne', 6: 'be', 7: 'a', 8: 's', 9: 'ns', 10: 'p', 11: 'np', 12: 'l', 13: 'ge', 14: 'le', 15: 'g'}
IA32_CONDITION_CODES = {name: code for code, name in IA32_CONDITION_NAMES.items()}
IA32_RELATIONAL_MIRROR = {'b': 'a', 'a': 'b', 'ae': 'be', 'be': 'ae', 'e': 'e', 'ne': 'ne', 'l': 'g', 'g': 'l', 'ge': 'le', 'le': 'ge'}
...
def _ia32_relational_flag_table() -> dict:
    ...
    table = {}
    for opcode in (136, 137, 138, 139, 140, 141, 142, 160, 161, 162, 163, 198, 199, 134, 135, 152, 153, 200, 201, 96, 97, 104, 106, 6, 14, 22, 30, 7, 23, 31, 252, 253):
        table[opcode] = quiet
...
def _ia32_relational_group_table() -> dict:
    ...
    """
    group1 = {digit: (frozenset({'cf'}) if digit in (2, 3) else frozenset(), IA32_ARITHMETIC_FLAGS) for digit in range(8)}
    group3 = {0: (frozenset(), IA32_ARITHMETIC_FLAGS), 1: (frozenset(), IA32_ARITHMETIC_FLAGS), 2: (frozenset(), frozenset()), 3: (frozenset(), IA32_ARITHMETIC_FLAGS), 4: (frozenset(), frozenset({'cf', 'of'})), 5: (frozenset(), frozenset({'cf', 'of'})), 6: (frozenset(), frozenset()), 7: (frozenset(), frozenset())}
    group5 = {digit: (frozenset(), IA32_ARITHMETIC_FLAGS - {'cf'} if digit in (0, 1) else frozenset()) for digit in range(8)}
    shift_one = {digit: (frozenset({'cf'}) if digit in (2, 3) else frozenset(), frozenset({'cf', 'of'})) for digit in range(8)}
    shift_var = {digit: (frozenset({'cf'}) if digit in (2, 3) else frozenset(), frozenset()) for digit in range(8)}
    return {128: group1, 129: group1, 131: group1, 246: group3, 247: group3, 254: group5, 255: group5, 208: shift_one, 209: shift_one, 192: shift_var, 193: shift_var, 210: shift_var, 211: shift_var}
...
def ia32_relational_flow_walk(body: bytes, relocations: dict | None, context: str, code_length: int | None=None, external_entries: frozenset | None=None) -> tuple[list[dict], list[list[int]], list[int]]:
    ...
    while offset < len(code):
        ...
        cursor = offset
        repeated = False
        while code[cursor] in _IA32_RELATIONAL_PREFIXES or code[cursor] in _IA32_RELATIONAL_REPEAT_PREFIXES:
            if code[cursor] in _IA32_RELATIONAL_REPEAT_PREFIXES:
                require(cursor + 1 < offset + length and (code[cursor + 1] in _IA32_RELATIONAL_STRING_QUIET or code[cursor + 1] in _IA32_RELATIONAL_STRING_COMPARE), f'{context}: the repeat prefix at {offset} does not prefix a string opcode')
                repeated = True
            cursor += 1
            require(cursor < offset + length, f'{context}: instruction at {offset} is only prefixes')
        opcode = code[cursor]
        require(opcode not in _IA32_RELATIONAL_REFUSED_OPCODES, f'{context}: opcode 0x{opcode:02x} at {offset} is outside the relational-form flow table')
        if opcode == 15:
            require(cursor + 1 < offset + length, f'{context}: truncated two-byte opcode at {offset}')
            opcode = 3840 | code[cursor + 1]
        flow, target, condition = ('fall', None, None)
        if 112 <= opcode <= 127:
            flow, condition = ('jcc', opcode & 15)
            target = offset + length + int.from_bytes(code[offset + length - 1:offset + length], 'little', signed=True)
        elif 3968 <= opcode <= 3983:
            flow, condition = ('jcc', opcode & 15)
            target = offset + length + int.from_bytes(code[offset + length - 4:offset + length], 'little', signed=True)
        elif opcode in (235, 233):
            flow = 'jmp'
            width = 1 if opcode == 235 else 4
            target = offset + length + int.from_bytes(code[offset + length - width:offset + length], 'little', signed=True)
        elif opcode == 232:
            flow = 'call'
            ...
            require(extension not in (3, 5), f'{context}: a far transfer at {offset} makes the control-flow graph unknowable')
            if extension == 2:
                flow = 'call'
            elif extension == 4:
                require(external_entries, f'{context}: a computed jump at {offset} makes the control-flow graph unknowable')
                flow = 'computed'
                target = None
        if flow in ('jmp', 'jcc') and (opcode == 233 or 3968 <= opcode <= 3983) and (relocations.get(offset + length - 4, {}).get('width') == 4):
        ...
        if flow == 'jcc' and condition is not None:
            ...
            reads, writes = IA32_RELATIONAL_FLAG_EFFECTS.get(opcode, (IA32_ARITHMETIC_FLAGS, frozenset()))
            if repeated and opcode in _IA32_RELATIONAL_STRING_COMPARE:
                reads = reads | {'zf'}
        items.append({'offset': offset, 'length': length, 'opcode': opcode, 'opcode_at': cursor, 'flow': flow, 'target': target, 'condition': condition, 'reads_flags': frozenset(reads), 'writes_flags': frozenset(writes)})
        offset += length
    require(offset == len(code), f'{context}: body does not decode to exhaustion')
    starts = {item['offset'] for item in items}
    for item in items:
        require(item['target'] is None or item['target'] in starts, f"{context}: the branch at {item['offset']} does not target an instruction boundary of this body")
    ...
    for index, item in enumerate(items):
        ...
        if item['flow'] == 'computed':
            edges.extend((index_of[target] for target in sorted(external_entries or ()) if target in index_of))
    ...
    for target in external:
        require(target in index_of, f'{context}: the external entry {target} is not an instruction boundary of this body')
    ...
    require(not unreachable, f'{context}: the instruction at {unreachable[:1]} is reachable neither from the entry nor from a declared external entry, so the control-flow graph is incomplete')
    return (items, successors, entries)

def ia32_relational_flag_liveness(items: list[dict], successors: list[list[int]], context: str) -> list[frozenset]:
    """Backward per-flag liveness, on the SAME fixpoint the register and web
    certificates use -- the flags are simply its atoms."""
    shim = [{'read_atoms': item['reads_flags'], 'write_atoms': item['writes_flags']} for item in items]
    return _ia32_backward_liveness(shim, successors, context)

def apply_relational_form(body: bytes, sites: list, relocation_offsets: frozenset, context: str, relocations: dict | None=None, code_length: int | None=None, external_entries: frozenset | None=None) -> tuple[bytes, dict]:
    ...
    for ordinal, site in enumerate(sites):
        ...
        require(compare_at in index_of and branch_at in index_of, f'{site_context}: an offset is not an instruction boundary')
        compare_index = index_of[compare_at]
        branch_index = index_of[branch_at]
        compare = items[compare_index]
        branch = items[branch_index]
        require(branch_index == compare_index + 1, f'{site_context}: the branch does not immediately follow the compare')
        require(compare['opcode'] in IA32_RELATIONAL_COMPARE_PAIRS, f'{site_context}: the instruction at {compare_at} is not a two-operand cmp with a reversible encoding')
        require(compare['flow'] == 'fall', f'{site_context}: the compare is a control transfer')
        require(branch['flow'] == 'jcc' and branch['condition'] is not None, f'{site_context}: the instruction at {branch_at} is not a conditional branch this table can mirror')
        require(predecessors[branch_index] == [compare_index], f'{site_context}: the branch has a predecessor other than its compare, so another path would consume flags this compare did not produce')
        decoded = decode_ia32_bijection_instruction(body, compare_at, f'{site_context} compare', relocations)
        require(decoded['opcode'] == compare['opcode'] and decoded['length'] == compare['length'], f'{site_context}: the two decoders disagree about the compare')
        ...
        require(seed_name in IA32_RELATIONAL_MIRROR, f"{site_context}: condition 'j{seed_name}' has no mirror -- it reads a flag whose value under the reversal is not a function of the original flags")
        image_name = IA32_RELATIONAL_MIRROR[seed_name]
        require(site['seed_condition'] == seed_name and site['image_condition'] == image_name, f"{site_context}: the declared condition pair is not the closed table's mirror")
        out = frozenset().union(*[live[edge] for edge in successors[branch_index]]) if successors[branch_index] else frozenset()
        offending = sorted(out & IA32_RELATIONAL_CHANGED_FLAGS)
        require(not offending, f'{site_context}: the reversal changes {offending} and a successor of the branch reads it')
        compare_byte = compare['opcode_at']
        branch_byte = branch['opcode_at'] + (1 if branch['opcode'] >= 3840 else 0)
        require(compare_byte not in relocation_offsets and branch_byte not in relocation_offsets, f'{site_context}: a rewritten byte overlaps a relocation')
        if site.get('reencode'):
            ...
            require(modrm_at not in relocation_offsets, f'{site_context}: a rewritten byte overlaps a relocation')
    ...
    require({index for index in range(len(body)) if body[index] != image[index]} <= set(rewritten), f'{context}: the image changed a byte no site declares')
    ...
    for site in proved:
        ...
        offending = sorted(out & IA32_RELATIONAL_CHANGED_FLAGS)
        require(not offending, f"{context}: after the rewrite a successor of the branch at {site['branch_offset']} reads {offending}")
    ...
    for entry in image_entries[1:]:
        ...
        require(not offending, f"{context}: after the rewrite the external entry at {image_items[entry]['offset']} has {offending} live")
    return (image, {'kind': RELATIONAL_FORM_KIND, 'sites': proved, 'instruction_count': len(items), 'rewritten_offsets': rewritten, 'external_entries': [items[index]['offset'] for index in entries[1:]], 'preserved_flags': sorted(IA32_RELATIONAL_PRESERVED_FLAGS), 'changed_flags': sorted(IA32_RELATIONAL_CHANGED_FLAGS)})
...
def relational_form_external_entries(obj: 'CoffObject', section: dict, context: str) -> frozenset:
    ...
    entries = {row['target_value'] for row in detailed_relocations(obj, section) if row.get('target_section') == number}
    ...
    for child_name in names:
        ...
        entries |= {row['target_value'] for row in detailed_relocations(obj, child) if row.get('target_section') == number}
    entries.discard(0)
...
def validate_relational_form(value: object, context: str, body_length: int) -> dict:
    ...
    exact_audit_keys(value, {'kind', 'sites', 'expected_instruction_count', 'expected_rewritten_offsets', 'expected_external_entries', 'expected_seed_debug_s_sha256', 'authenticity_rationale', 'expected_code_length'}, context, optional={'expected_code_length'})
    ...
    for index, site in enumerate(sites):
        ...
        exact_audit_keys(site, {'compare_offset', 'branch_offset', 'seed_condition', 'image_condition'}, site_context)
        ...
        normalized_sites.append({'compare_offset': compare_at, 'branch_offset': branch_at, 'seed_condition': seed_condition, 'image_condition': IA32_RELATIONAL_MIRROR[seed_condition]})
    offsets = value.get('expected_rewritten_offsets')
    require(isinstance(offsets, list) and len(offsets) == 2 * len(normalized_sites) and (offsets == sorted(set(offsets))) and all((type(offset) is int and 0 <= offset < body_length for offset in offsets)), f'{context}.expected_rewritten_offsets is invalid')
    ...
    normalized = {'kind': RELATIONAL_FORM_KIND, 'sites': normalized_sites, 'expected_instruction_count': require_exact_int(value.get('expected_instruction_count'), f'{context}.expected_instruction_count', minimum=2), 'expected_rewritten_offsets': list(offsets), 'expected_external_entries': list(external), 'expected_seed_debug_s_sha256': require_sha(value.get('expected_seed_debug_s_sha256'), f'{context}.expected_seed_debug_s_sha256'), 'authenticity_rationale': rationale}
    code_length = value.get('expected_code_length')
    if code_length is not None:
        normalized['expected_code_length'] = require_exact_int(code_length, f'{context}.expected_code_length', minimum=2, maximum=body_length)
    return normalized

def produce_relational_form_candidate(seed_bytes: bytes, donor_bytes: bytes, function: dict) -> tuple[bytes, dict]:
    ...
    require(function.get('splice_class') == RELATIONAL_FORM_CLASS, 'splice class is not retail_exact_relational_form')
    require('target_source_refactor' not in function, 'relational-form functions carry no source refactor')
    ...
    require(sp['number'] == dp['number'] == function['expected_section_number'], 'relational-form target section seat changed')
    require(len(seed.sections) == len(donor.sections) == function['expected_section_count'], 'relational-form global section count changed')
    seed_functions = function_multiset(seed)
    donor_functions = function_multiset(donor)
    require(seed_functions == donor_functions and sum(seed_functions.values()) == function['expected_function_count'], 'relational-form donor function set differs')
    seed_comdats = comdat_primary_identity_multiset(seed)
    donor_comdats = comdat_primary_identity_multiset(donor)
    require(seed_comdats == donor_comdats and sum(seed_comdats.values()) == function['expected_comdat_count'], 'relational-form donor COMDAT identity set differs')
    require(sp['raw_size'] == dp['raw_size'] == function['expected_body_length'] and sp['relocation_count'] == dp['relocation_count'] == function['expected_relocation_count'] and (sp['line_count'] == function['expected_seed_line_count']) and (dp['line_count'] == function['expected_donor_line_count']) and (sp['name'] == dp['name']) and (sp['characteristics'] == dp['characteristics'] == function['expected_characteristics']), 'relational-form target header/count pins changed')
    require(section_definitions(seed)[sp['number']]['selection'] == section_definitions(donor)[dp['number']]['selection'] == function['expected_selection'], 'relational-form COMDAT selection changed')
    expected_closure = tuple(function['expected_closure'])
    require(_comdat_child_closure(seed, sp) == _comdat_child_closure(donor, dp) == (len(expected_closure), expected_closure), 'relational-form target closure changed')
    require(list(expected_closure) in (RELATIONAL_FORM_FPO_CLOSURE, RELATIONAL_FORM_EH_CLOSURE), 'relational-form closure pin names no installation delegate')
    delegate = relational_form_delegate(function['expected_closure'], function['expected_code_renames'])
    require(instruction_mosaic_metadata_sha256(seed, sp) == function['expected_seed_metadata_sha256'] and instruction_mosaic_metadata_sha256(donor, dp) == function['expected_donor_metadata_sha256'], 'relational-form metadata differs from its pin')
    seed_body = coff_body(seed, sp)
    donor_body = coff_body(donor, dp)
    require(sha256_bytes(seed_body) == function['expected_seed_body_sha256'] and sha256_bytes(donor_body) == function['expected_donor_body_sha256'], 'relational-form seed/donor body differs from its pin')
    seed_rows = detailed_relocations(seed, sp)
    donor_rows = detailed_relocations(donor, dp)
    code_renames = require_instruction_mosaic_semantic_relocations(seed, sp, donor, dp, 'relational-form code')
    require([[offset, kind] for offset, kind in code_renames] == function['expected_code_renames'], 'relational-form code rename set changed')
    require(len(seed_rows) == len(donor_rows) and [(row['offset'], row['type'], row['addend']) for row in seed_rows] == [(row['offset'], row['type'], row['addend']) for row in donor_rows], 'relational-form donor relocation layout differs from the seed')
    require([row['target'] for row in seed_rows if row['type'] == 20] == [row['target'] for row in donor_rows if row['type'] == 20], 'relational-form donor call/branch relocation targets differ from the seed')
    ...
    require(external == relational_form_external_entries(donor, dp, 'relational-form donor'), 'relational-form donor external entry set differs from the seed')
    require(sorted(external) == spec['expected_external_entries'], 'relational-form external entry set changed')
    image, proof = apply_relational_form(donor_body, spec['sites'], relocation_offsets, 'relational-form image', relocation_symbols, spec.get('expected_code_length'), external)
    require(proof['rewritten_offsets'] == spec['expected_rewritten_offsets'] and proof['instruction_count'] == spec['expected_instruction_count'], 'relational-form image differs from its declaration')
    require(sha256_bytes(image) == function['expected_body_sha256'], 'relational-form image differs from its pin')
    ...
    require(composed_rows == installed_rows and [row['symbol_index'] for row in composed_rows] == [row['symbol_index'] for row in seed_rows] and (_coff_table_bytes(checked, cp, 'relocations') == _coff_table_bytes(seed, sp, 'relocations')) and (_coff_table_bytes(checked, cp, 'lines') == _coff_table_bytes(seed, sp, 'lines')), 'relational-form output changed seed relocation/line bytes')
    debug_child = _comdat_child(checked, cp, '.debug$S')
    require(sha256_bytes(coff_body(checked, debug_child)) == spec['expected_seed_debug_s_sha256'], 'relational-form debug$S differs from its pin')
    for child_name in expected_closure:
        require(coff_body(checked, _comdat_child(checked, cp, child_name)) == coff_body(seed, _comdat_child(seed, sp, child_name)), f'relational-form output changed its {child_name} child')
    boundaries = {item['offset'] for item in ia32_relational_flow_walk(image, relocation_symbols, 'relational-form image lines', spec.get('expected_code_length'), external)[0]}
    if cp['line_count'] > 1:
        ...
        for index in range(1, cp['line_count']):
            ...
            require(row_line != 0 and row_offset in boundaries, f'relational-form line row at {row_offset} does not land on an image instruction boundary')
    allowed = set(range(sp['raw_offset'], sp['raw_offset'] + sp['raw_size']))
    require({index for index in range(len(seed_bytes)) if seed_bytes[index] != composed[index]} <= allowed, 'relational-form changed bytes outside its own COMDAT')
    return (composed, {**detail, 'splice_class': RELATIONAL_FORM_CLASS, 'relational_form': proof['sites'], 'instruction_count': proof['instruction_count'], 'rewritten_offsets': proof['rewritten_offsets'], 'external_entries': proof['external_entries'], 'preserved_flags': proof['preserved_flags'], 'changed_flags': proof['changed_flags'], 'candidate_only': True, **semantic_detail})
