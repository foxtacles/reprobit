...
def _fp_sum_parse_chain(body: bytes, start: int, end: int, context: str) -> list[dict]:
    ...
    while offset < end:
        ...
        require(
            offset + length <= end,
            f"{context}: an instruction straddles the chain boundary at {offset}",
        )
        ...
        if opcode == 221 and length >= 2 and (encoded[1] >> 6 != 3) and (encoded[1] >> 3 & 7 == 0):
            require(
                pending_fld is None,
                f"{context}: two fld in a row at {offset} -- not a product pair",
            )
            ...
            require(
                False,
                f"{context}: the instruction at {offset} is not an fld m64, fmul m64 or faddp st(1)",
            )
        offset += length
...
def apply_x87_squared_addend_exchange(
    ...
    for ordinal, chain in enumerate(chains):
        ...
        require(
            not (start - 1 in relocation_offsets and start in relocation_offsets)
            and (not (end - 1 in relocation_offsets and end in relocation_offsets)),
            f"{chain_context}: a relocated record crosses the chain edge",
        )
        carried_runs = []
        run_start = None
        ...
        for run_lo, run_hi in carried_runs:
            require(
                ...
                f"{chain_context}: a relocated run inside the chain is not one whole record",
            )
        units = []
        cursor = start
        while cursor < end:
            require(
                ...
                f"{chain_context}: the instruction at {cursor} is not an fld m32",
            )
            ...
            require(
                ...
                f"{chain_context}: the instruction at {sub_at} is not an fsub/fadd m32",
            )
            sub_len = _x87_m32_length(body, sub_at, chain_context)
            units.append((cursor, fld_len + sub_len, fld_len))
            cursor = sub_at + sub_len
        require(
            cursor == end and len(units) >= 2,
            f"{chain_context}: the window is not a whole number of fld/fsub units",
        )
        require(
            all(
                (
                    any(
                        (
                            offset <= run_lo and run_hi <= offset + length
        ...
        while True:
            require(
                scan + 2 <= len(body), f"{chain_context}: the consumption region runs off the body"
            )
            ...
            if len(stack) == 1 and (not isinstance(stack[0], int)):
                break
        require(
            len(stack) == 1 and (not isinstance(stack[0], int)) and all(squared),
            f"{chain_context}: the consumption region does not fold every squared unit into one commutative sum",
        )
        fold_tree = stack[0]
        order = chain["order"]
        require(
            isinstance(order, list) and sorted(order) == list(range(len(units))),
            f"{chain_context}: the order is not a permutation of the {len(units)} units",
        )
        ...
        require(
            canonical(fold_tree, order) == canonical(fold_tree, identity),
        ...
        require(
            rewritten == chain["expected_rewritten_offsets"],
            f"{chain_context}: rewrote a different byte set from its declaration",
        )
        require(
            sorted(map(list, chain_reseat))
            == sorted(map(list, chain.get("relocation_reseat") or [])),
            f"{chain_context}: reseated a different relocation set from its declaration",
        )
        proved.append(
            {
                "chain_start": start,
...
def apply_fp_sum_reassociation(
    ...
    for ordinal, chain in enumerate(chains):
        ...
        order = chain["order"]
        require(
            isinstance(order, list) and sorted(order) == list(range(len(pair_slots))),
            f"{chain_context}: the order is not a permutation of the {len(pair_slots)} product pairs",
        )
