...
def parse_codeview_symbol_stream(data: bytes, context: str) -> list[dict]:
    ...
    while offset < len(data):
        ...
        total = length + 2
        require(
            length >= 2 and offset + total <= len(data),
            f"{context}: symbol record length is out of range",
        )
        record_type = int.from_bytes(data[offset + 2 : offset + 4], "little")
        require(
            record_type in CODEVIEW_SYMBOL_NAME_OFFSETS,
            f"{context}: symbol record type 0x{record_type:04x} is outside the closed table",
        )
        ...
        if name_offset is None:
            ...
            require(
                len(payload) == name_offset + 1 + count,
...
DEBUG_REPRESENTATION_DELTA_KINDS = frozenset(
    {"procedure_extent", "compiler_label_number", "local_location", "inserted_donor_local"}
)
...
def _codeview_local_location(record_type: int, payload: bytes, context: str) -> tuple[int, dict]:
    ...
    return (
        ...
        {"bp_offset": int.from_bytes(payload[0:4], "little", signed=True)},
    )
...
def validate_debug_representation_delta(value: object, context: str) -> list[dict]:
    ...
    for index, item in enumerate(value):
        ...
        kind = item.get("kind")
        require(
            kind in DEBUG_REPRESENTATION_DELTA_KINDS,
        ...
        record_index = require_exact_int(
            item.get("record_index"), item_context + ".record_index", minimum=0, maximum=1 << 12
        )
        ...
        if kind == "local_location":
            ...
            if "seed_type" in item or "donor_type" in item:
                for type_key in ("seed_type", "donor_type"):
                    normalized_item[type_key] = require_exact_int(
                        item.get(type_key), item_context + "." + type_key, minimum=0, maximum=65535
                    )
            else:
            ...
            for side in ("seed_location", "donor_location"):
                ...
                if key == "register":
                    bound = require_exact_int(
                        location[key], f"{item_context}.{side}.register", minimum=1, maximum=65535
                    )
                else:
                    bound = location[key]
                    require(
                        type(bound) is int and bound != 0 and (-(1 << 31) <= bound < 1 << 31),
                        f"{item_context}.{side}.bp_offset is invalid",
                    )
                normalized_item[side] = {key: bound}
            moves_type = (
                "seed_type" in normalized_item
                and normalized_item["seed_type"] != normalized_item["donor_type"]
            )
            require(
                normalized_item["seed_location"] != normalized_item["donor_location"] or moves_type,
            ...
            if key == "register":
                bound = require_exact_int(
                    location[key], f"{item_context}.location.register", minimum=1, maximum=65535
                )
            else:
                bound = location[key]
                require(
...
def require_debug_symbol_representation_delta(
    ...
    for index, donor_record in enumerate(donor_records):
        ...
        if item is not None and item["kind"] == "inserted_donor_local":
            ...
            require(
                donor_record["type"] in CODEVIEW_LOCAL_LOCATION_RECORD_TYPES,
                f"{context}: inserted record {index} is outside the closed local-location forms",
            )
            require(
                donor_record["name"] == item["name"],
                f"{context}: inserted record {index} name differs from its pin",
            )
            donor_type, donor_location = _codeview_local_location(
                donor_record["type"], donor_payload, f"{context} (donor)"
            )
            require(
                donor_type == item["type"] and donor_location == item["location"],
                f"{context}: inserted record {index} differs from its declaration",
            )
            detail.append(
                {
                    "record_index": index,
        ...
        if kind == "procedure_extent":
            ...
            require(
                seed_length == expected_seed_length and donor_length == expected_donor_length,
                f"{context}: record {index} extent differs from the row's length pins",
            )
            require(
                seed_start <= seed_end <= seed_length and donor_start <= donor_end <= donor_length,
                f"{context}: record {index} debug range does not sit inside its own extent",
            )
        elif kind == "compiler_label_number":
            require(
                seed_record["type"] == donor_record["type"] == 521
                and seed_payload[:7] == donor_payload[:7],
                f"{context}: record {index} is not a label pair",
            )
            require(
                local_symbol_kind(seed_record["name"]) == "L"
                and local_symbol_kind(donor_record["name"]) == "L",
                f"{context}: record {index} is not a compiler-numbered label",
            )
        else:
            require(
                seed_record["type"] in CODEVIEW_LOCAL_LOCATION_RECORD_TYPES
                and donor_record["type"] in CODEVIEW_LOCAL_LOCATION_RECORD_TYPES,
                f"{context}: record {index} is outside the closed local-location forms",
            )
            require(
                seed_record["name"] == donor_record["name"] == item["name"],
            ...
            if "type" in item:
                require(
                    seed_type == donor_type == item["type"],
                    f"{context}: record {index} type index differs from its pin",
                )
            else:
                require(
                    seed_type == item["seed_type"] and donor_type == item["donor_type"],
                    f"{context}: record {index} type index differs from its pin",
                )
            require(
                seed_location == item["seed_location"] and donor_location == item["donor_location"],
                f"{context}: record {index} location differs from its declaration",
            )
        detail.append({"record_index": index, "kind": kind, "name": seed_record["name"]})
    require(
        seed_cursor == len(seed_records),
        f"{context}: the seed stream has records the walk never paired",
    )
    return detail
...
def require_removed_caller_locals_delta(
    ...
    require(
        len(seed_stream) == delta["expected_seed_debug_size"]
    ...
    for item in removed:
        ...
        require(
            ...
            f"{context}: pinned removed record at {item['seed_offset']} is not the seed's {item['identifier']!r}",
        )
        require(
            item["seed_offset"] >= guard,
            f"{context}: a removed record overlaps a relocated span, so debug$S relocation offsets would move",
        )
    ...
    require(
        [codeview_symbol_identity(item) for item in surviving]
        == [codeview_symbol_identity(item) for item in donor_records],
        f"{context}: the seed symbol stream minus its pinned removals is not the donor's symbol sequence",
    )
...
def parse_fpo_data(raw: bytes, *, expected_proc_size: int | None = None) -> dict:
    ...
    if expected_proc_size is not None:
        require(
            type(expected_proc_size) is int
            and expected_proc_size > 0
...
SOURCE_FPO_CHILD_IDENTITY_KEYS = {
    "section_number",
    "expected_seed_raw_size",
    "expected_donor_raw_size",
    "relocation_count",
    "line_count",
    "characteristics",
    "selection",
