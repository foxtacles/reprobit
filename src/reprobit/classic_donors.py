...
class DonorCompilerAdditions:
    """Typed, platform-independent additions to an admitted compiler lane."""
...
def _carrier_identity(parameters: Mapping[str, object], generated: bytes) -> None:
    ...
    legacy_id = parameters.get("legacy_recipe_id")
    _require(legacy_id == f"d_{digest[:12]}", "legacy recipe id differs from generated bytes")


def _compile_lane(parameters: Mapping[str, object], *, overlay: bool) -> tuple[str, str | None]:
    lane = parameters.get("compile_lane")
    _require(isinstance(lane, Mapping), "compile_lane must be an object")
    lane_map = cast(Mapping[str, object], lane)
    expected = {"required_define", "include_projection"} if overlay else {"required_define"}
    _exact_keys(
        lane_map,
        expected,
        "compile_lane",
        optional={"include_projection"},
    )
    required_define = _identifier(
        lane_map.get("required_define"), "compile_lane.required_define"
    )
    projection = lane_map.get("include_projection")
    _require(
        projection is None or projection in _PROJECTIONS,
        "compile_lane.include_projection is outside the closed enum",
    )
    return required_define, cast(str | None, projection)
...
def _validated_overlay(
    ...
    optional = {"canonical_overlay_replay", "compiler_state_carrier"}
    ...
    _require(
        parameters.get("legacy_recipe_id") == f"d_{identity[:12]}",
        "legacy recipe id differs from rendering identity",
    )
    ...
    return DonorRecipeValidation(
        ...
        cast(Mapping[str, object], _freeze(parameters)),
        required_define,
        DonorIncludeProjection(projection or "none"),
        None,
        force_payload,
        carrier_identifiers,
    )
...
def validate_donor_recipe(
    ...
    required_define, projection = _compile_lane(
        parameters, overlay=intervention.family is ClassicRecipeFamily.DONOR_SOURCE_OVERLAY
    )
    if intervention.family is ClassicRecipeFamily.DONOR_SOURCE_OVERLAY:
        return _validated_overlay(intervention, parameters, required_define, projection)
    _require(projection is None, "ordinary donor cannot project includes")
    _require(
    ...
    generated: bytes
    force_payload: bytes | None = None
    common = {"compile_lane", "emission_policy", "generated_header_sha256", "legacy_recipe_id"}
...
def prepare_donor_compile_request(
    ...
    *,
    ...
    overlay_dialect: ClassicOverlayDialect | None = None,
) -> DonorCompileRequest:
    ...
    compiler_additions = DonorCompilerAdditions(
        validation.required_define,
        force_includes,
        include_directories,
        validation.include_projection,
    )
    carrier_identifiers = validation.carrier_identifiers
    additions_material = {
        "required_define": compiler_additions.required_define,
    ...
    return DonorCompileRequest(
        intervention.id,
        cast(str, parameters["legacy_recipe_id"]),
        intervention.family,
